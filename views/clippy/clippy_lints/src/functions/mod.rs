...
declare_clippy_lint! {
    ...
    ///
    /// Only return positions count. Parenthesised `Fn`-trait sugar lowers the
    /// callable's argument list into a tuple (`FnOnce(A, B)` is `FnOnce<(A, B)>`),
    /// and a nested function pointer has parameters of its own; those tuples are
    /// arguments rather than returned values, so they are not linted. The sugar's
    /// `Output` type is a return position and is still checked.
    ///
    /// ### Why restrict this?

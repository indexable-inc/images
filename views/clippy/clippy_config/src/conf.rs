...
define_Conf! {
    ...
    /// Method names that count as attaching a span to a future, for the purpose of
    /// `uninstrumented_await`. Defaults to `tracing`'s own combinators; add a project's
    /// equivalent here rather than shipping a new clippy.
    #[lints(uninstrumented_await)]
    instrumenting_methods: Vec<String> = vec!["instrument".to_string(), "in_current_span".to_string(), "awaited".to_string()],

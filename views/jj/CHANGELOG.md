...
* `jj views push` now refuses a commit that changes paths outside the view it is
  publishing. A view filters the files it sends and copies the commit message
  through unchanged, so a commit spanning both halves published a message
  describing work the receiving repository cannot see. The refusal names the
  commit and the paths and suggests `jj split`; `--allow-mixed` keeps the old
  behavior. The new `jj views check` runs the same scan without pushing and
  exits non-zero when it finds anything.

* `jj sparse set` no longer costs the size of the repository when it narrows the
  working copy. Removing files was a tree diff whose matcher visited every
  directory the new patterns did not cover, reading one tree object per
  directory, sequentially, to rediscover paths the working copy could already
  name. It now deletes by path from the working copy's own file states and reads
  no tree objects at all.
...
* New `jj views push --replace-drifted` reconciles a published branch that holds
  its own copies of commits this repository already derives -- the same trees
  under different commit objects, because both sides created the work
  independently. Nothing else could: `jj views fetch` compares content, finds
  nothing missing and reports the view up to date, while `jj views anchor`
  compares hashes and fails. The flag only replaces commits whose tree the
  derived tip already has, so a branch carrying content this repository does not
  produce is refused rather than overwritten, and it writes the replaced tip to
  `refs/pins/<date>-<hash>` in the same push, for the lock files elsewhere that
  pin it. Pushing to a view's own default branch now refuses any replacement
  that is not this one, instead of forcing it.
...
* `jj views push`, `jj views fetch` and `jj views status` no longer validate
  views they were not asked to act on. Seeding a view's anchor reads the commit
  object the manifest names, so one entry whose anchor had rotted -- a commit
  its published branch no longer reaches and no endpoint will serve by hash --
  failed every one of these commands in the repository, including one naming a
  single healthy view. They now check and seed only the views named on the
  command line, or every view when none is named. `jj views tree` and an
  unqualified `jj views status` still answer for the whole manifest.

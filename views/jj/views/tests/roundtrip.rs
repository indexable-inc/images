...
fn write_base(repo: &gix::Repository) -> ObjectId {
    ...
    // `vendor.txt` sits next to the `vendor` directory the graft rebuilds, and it
    // is the pair that makes tree ordering observable: byte order puts `vendor`
    // first, git's rule compares the directory as `vendor/` and `.` (0x2e) sorts
    // below `/` (0x2f), so `vendor.txt` comes first. Without a discriminating pair
    // at a rebuilt level, a wrong sort produces identical hashes and the pinned
    // hashes below gate nothing about rule 9.
    let mut entries = vec![
        gix::objs::tree::Entry {
...
#[test]
fn a_valid_anchor_derives_only_commits_after_its_source() {
    let filter = Filter::prefix(PREFIX).expect("a valid prefix");
    let world = inject(&filter);
    let source = fixture_commit(&world, "odd-timezone-offset");
    let source_in_parent = world.map[&source];
    let revision = world.map[&fixture_commit(&world, "modes-symlink-gitlink")];

    let mut cold = Cache::new();
    let cold_tip = jj_views::derive(&world.repo, &revision, &filter, &mut cold)
        .expect("a cold derivation")
        .expect("a view tip");

    let mut anchored = Cache::new();
    anchored
        .seed_anchor(
            &world.repo,
            &revision,
            &filter,
            DeriveAnchor {
                source: source_in_parent,
                view: source,
            },
        )
        .expect("a valid anchor");
    let anchored_tip = jj_views::derive(&world.repo, &revision, &filter, &mut anchored)
        .expect("an anchored derivation")
        .expect("a view tip");

    assert_eq!(anchored_tip, cold_tip, "the anchor changed the result");
    assert_eq!(
        anchored.commit_entries(),
        3,
        "the anchor plus the two descendants should be the whole cache"
    );
    assert_eq!(
        cold.commit_entries(),
        9,
        "the cold derivation should traverse the fixture history"
    );
}

#[test]
fn thirty_two_exact_tips_skip_exhaustive_derivation_and_share_one_host_graph_walk() {
    let filter = Filter::prefix(PREFIX).expect("a valid prefix");
    let world = inject(&filter);
    let source = fixture_commit(&world, "odd-timezone-offset");
    let revision = world.map[&fixture_commit(&world, "modes-symlink-gitlink")];
    ...
    let expected = {
        let mut cache = Cache::new();
        cache
            .seed_anchor(&world.repo, &revision, &filter, anchor)
            .expect("the anchor is valid");
        jj_views::derive_tip(&world.repo, &revision, &filter, &mut cache)
            .expect("the exact tip derives")
    ...
    for _ in 0..32 {
        cache
            .seed_anchor(&world.repo, &revision, &filter, anchor)
            .expect("the shared anchor is valid");
        let tip = jj_views::derive_tip(&world.repo, &revision, &filter, &mut cache)
            .expect("the tip derives");
        if tip != Some(expected) {
            jj_views::derive(&world.repo, &revision, &filter, &mut cache)
                .expect("a mismatched tip derives exhaustively");
        }
        cache.discard_filter(&filter);
    }

    assert_eq!(
        cache.exhaustive_derivations(),
        0,
        "32 exact tips must not request an exhaustive derivation"
    );
    assert_eq!(
        cache.ancestry_walks(),
        1,
        "32 views of one host revision must traverse its graph once"
    );
}

#[test]
fn an_anchor_with_a_different_filtered_tree_is_refused() {
    let filter = Filter::prefix(PREFIX).expect("a valid prefix");
    let world = inject(&filter);
    let source = fixture_commit(&world, "odd-timezone-offset");
    let revision = world.map[&world.upstream.head];
    let wrong_view = fixture_commit(&world, "root");
    ...
    let error = Cache::new()
        ...
        .expect_err("different trees cannot seed derivation");
    assert!(
        matches!(error, jj_views::Error::AnchorTreeMismatch { .. }),
        "unexpected error: {error}"
    );
}

#[test]
fn an_anchor_outside_the_requested_history_is_refused() {
    let filter = Filter::prefix(PREFIX).expect("a valid prefix");
    let world = inject(&filter);
    let source = fixture_commit(&world, "left");
    let requested = fixture_commit(&world, "right");
    ...
    let error = Cache::new()
        ...
        .expect_err("a side branch cannot anchor its sibling");
    assert!(
        matches!(error, jj_views::Error::AnchorNotAncestor { .. }),
        "unexpected error: {error}"
    );
}

#[test]
fn an_anchor_with_a_missing_object_is_refused() {
    let filter = Filter::prefix(PREFIX).expect("a valid prefix");
    let world = inject(&filter);
    let source = fixture_commit(&world, "odd-timezone-offset");
    let missing =
        ObjectId::from_hex(b"1111111111111111111111111111111111111111").expect("a full object id");

    let error = Cache::new()
        .seed_anchor(
            &world.repo,
            &world.map[&world.upstream.head],
            &filter,
            DeriveAnchor {
                source: world.map[&source],
                view: missing,
        ...
        .expect_err("a missing view commit cannot seed derivation");
    assert!(
        matches!(error, jj_views::Error::Find { id, .. } if id == missing),
        "unexpected error: {error}"
    );
}

fn fixture_commit(world: &Injected, label: &str) -> ObjectId {
    world
        .upstream
        .commits
        .iter()
        .find(|(candidate, _)| *candidate == label)
        .map(|(_, commit)| *commit)
        .unwrap_or_else(|| panic!("the fixture has no commit labeled {label}"))
}
...
fn semantics_v1_output_hashes_are_pinned() {
    ...
    let pinned = [
        (
            ...
            "f6ea86a3d519586649e0840210eb63fa97c85077 1492b8046406fe647585f6163e22766eb0a75317 \
             4d4882f724bab087bd4f9efacf8dd2ef9f9dd433",
        ),
        (
            "semantics=1;prefix=vendor/upstream;elide=nothing;trivial-merges=drop",
            "f6ea86a3d519586649e0840210eb63fa97c85077 b5ec0a8000170517de86112dacea8cfd4bd7a9cc \
             4d4882f724bab087bd4f9efacf8dd2ef9f9dd433",
        ),
        (
            "semantics=1;prefix=vendor/upstream;elide=unchanged;trivial-merges=keep",
            "f6ea86a3d519586649e0840210eb63fa97c85077 2af81485c5f516532bde4f4aedcf1e4d9d6774be \
             4d4882f724bab087bd4f9efacf8dd2ef9f9dd433",
        ),
    ];
    ...
    for (spec, _) in &pinned {
        ...
        // A commit that DELETES the filtered path, then one that restores it.
        // Without these, the rule that an absent path filters to the empty tree is
        // never reached by this scenario and the pinned hashes gate nothing about
        // it: changing that rule leaves them untouched.
        let deleted = append_commit(
            &world.repo,
            &[trivial],
            &write_path(&world.repo, &tree_of(&world.repo, &trivial), PREFIX, None),
        );
        let tip = append_commit(
            &world.repo,
            &[deleted],
            &write_path(
                &world.repo,
                &tree_of(&world.repo, &deleted),
                PREFIX,
                Some(vendored),
            ),
        );
        ...
        let mut cache = Cache::new();
        let derived = jj_views::derive(&world.repo, &tip, &filter, &mut cache)
            .expect("a derivation")
            .expect("the tip has a view");

        // A lift performed AFTER deriving, which is the only shape the lifting
        // rules 12 and 13 govern. Injection reaches `unfilter` too, and the
        // injected id below pins it, but injection never derives first, so the
        // view commit has exactly one counterpart and the rule choosing among
        // several is never exercised. That is not hypothetical: a change to that
        // rule passed this very test, and ENG-11639 is why this block is here.
        //
        // `moved` changes nothing under the prefix, so it elides onto `derived`
        // and gives that view commit a second counterpart, `tip` being the first.
        let moved = append_monorepo_commit(&world.repo, &tip, 3);
        jj_views::derive(&world.repo, &moved, &filter, &mut cache)
            .expect("a derivation")
            .expect("the moved head has a view");
        let patch = append_commit(&world.repo, &[derived], &vendored_subtree(&world, "right"));
        let lifted = jj_views::unfilter(&world.repo, &patch, &moved, &filter, &mut cache)
            .expect("a prefix graft cannot conflict on content");

        // Three ids, gating three different families of rule. The derived id
        // gates what shapes the view. The injected id gates what only shapes the
        // monorepo side, tree entry ordering and entry modes among them, which no
        // derived hash can observe because deriving never rebuilds an upstream
        // tree. The lifted id gates where a lift lands once a view commit has
        // more than one counterpart, which neither of the others can see.
        actual.push(format!("{head} {derived} {lifted}"));
    }
...
fn a_spec_this_version_cannot_read_is_refused() {
    for spec in [
        // A semantics version from the future. Moves as versions are added,
        // which is the point: the assertion is about an unknown number, and a
        // number that has since become known would make it assert nothing.
        "semantics=3;prefix=vendor/upstream;elide=unchanged;trivial-merges=drop",
...
/// Every version this build claims to know must actually round trip through a
/// spec, or the refusal test above is asserting against a list that does not
/// describe the build.
#[test]
fn every_known_semantics_version_round_trips_through_a_spec() {
    for version in Semantics::KNOWN {
        let spec = format!(
            "semantics={version};prefix=vendor/upstream;elide=unchanged;trivial-merges=drop"
        );
        let filter = Filter::parse(&spec).expect("a version this build knows");
        assert_eq!(
            filter.spec(),
            spec,
            "a known version must come back as it went in"
        );
    }
}

/// The two lifting rules are different rules, which is the whole reason
/// [`Semantics::V2`] exists rather than the change being made in place.
///
/// One setup, both versions, and they must disagree. Without this the pair of
/// tests above could both pass while the version flag did nothing, since each
/// only ever sees one version.
#[test]
fn v1_and_v2_lift_the_same_patch_to_different_places() {
    let v1 = Filter::prefix(PREFIX).expect("a valid prefix");
    let v2 = v1.clone().semantics(Semantics::V2);
    let world = inject(&v1);
    let injected_head = *world
        .map
        .get(&world.upstream.head)
        .expect("the head was injected");
    let monorepo_head = append_monorepo_commit(&world.repo, &injected_head, 11);

    let lift = |filter: &Filter| {
        let mut cache = Cache::new();
        let view_head = jj_views::derive(&world.repo, &monorepo_head, filter, &mut cache)
            .expect("a derivation")
        ...
        let lifted = jj_views::unfilter(&world.repo, &patch, &monorepo_head, filter, &mut cache)
            .expect("a prefix graft cannot conflict on content");
        parents_of(&world.repo, &lifted)
    };

    assert_eq!(
        lift(&v1),
        vec![injected_head],
        "V1 takes the counterpart the cache learned first"
    );
    assert_eq!(
        lift(&v2),
        vec![monorepo_head],
        "V2 takes the one the caller named"
    );
}
...
fn a_divergent_view_commit_survives_unfilter_then_derive() {
    ...
    // V1 lifting, which is the default: the counterpart used is whichever the
    // cache learned first, and `onto` gets no say even where it shares a view.
    // The sibling test below is this same shape under V2, where it does.
    assert_eq!(
        parents_of(&world.repo, &lifted),
        vec![injected_head],
...
/// The [`Semantics::V2`] rule that keeps a round trip from growing a commit
/// nobody wrote.
///
/// Same shape as the test above, under V2 rather than V1: the monorepo head has
/// been derived, so the cache knows its view is the very commit the patch was
/// authored against. Several monorepo commits can share one view commit, and
/// the graft map remembers the earliest, which after an import is the injected
/// commit buried in the vendored lineage. Lifting onto that puts the result
/// beside the monorepo instead of on it, and the merge needed to bring it back
/// reverse-maps into a merge commit in the view, one per round trip. Where the
/// caller's `onto` has the same view, the two are interchangeable as ancestry,
/// so the caller's is the one to take.
#[test]
fn a_lift_continues_the_monorepo_head_that_shares_its_view() {
    ...
    let mut cache = Cache::new();
    // Deriving the monorepo head rather than the injected one. It changed
    // nothing under the prefix, so it elides and its view IS the injected
    // head's view.
    let view_head = jj_views::derive(&world.repo, &monorepo_head, &filter, &mut cache)
        .expect("a derivation")
        .expect("the head has a view");
    ...
    assert_eq!(
        parents_of(&world.repo, &lifted),
        vec![monorepo_head],
        "the monorepo head derives to the patch's own parent, so the lift continues it and lands \
         on the monorepo tip rather than beside it"
    );

    // Carrying the monorepo head's content outside the prefix is inheritance
    // here, not the fabrication the test above guards against: this commit's
    // parent IS the monorepo head, so those changes are its parent's and are
    // not attributed to it.
    assert_eq!(
        entry_oid(&world.repo, &tree_of(&world.repo, &lifted), "MONOREPO"),
        entry_oid(
            &world.repo,
            &tree_of(&world.repo, &monorepo_head),
            "MONOREPO"
        ),
        "the lift must carry its parent's content outside the prefix, and its parent is the \
         monorepo head"
    );

    // The payoff, and the reason the rule is worth having: the round trip adds
    // no commit of its own, so the view does not accumulate merges.
    let round_tripped = jj_views::derive(&world.repo, &lifted, &filter, &mut cache)
        .expect("a derivation")
        .expect("the lifted commit has a view");
    assert_eq!(
        round_tripped, patch,
        "deriving the lift must give back the patch itself, byte for byte"
    );
}
...
/// The rules must be legible from the repository alone. A consumer holding a
/// derived history has to be able to answer "which semantics is this under"
/// without knowing which binary produced it.
#[test]
fn the_rules_are_recorded_in_the_tree_and_read_back() {
    let dir = tempfile::tempdir().expect("a temporary directory");
    let path = dir.path().join(jj_views::SPEC_FILE_NAME);

    let filter = Filter::prefix("vendor/linux")
        .expect("a valid prefix")
        .keep_trivial_merges(true);
    filter.write_spec(&path).expect("the spec is writable");

    let body = std::fs::read_to_string(&path).expect("the spec is readable");
    assert!(
        body.contains("semantics=1;prefix=vendor/linux;elide=unchanged;trivial-merges=keep"),
        "the file must carry the whole rule set, got:\n{body}"
    );

    let read = Filter::read_specs(&path).expect("the spec is parseable");
    assert_eq!(
        read,
        vec![filter],
        "the recorded rules must read back identically"
    );
}

/// A version this build does not implement is refused by name, so an operator
/// sees what to install rather than getting a history derived under the wrong
/// rules. A version it does implement is applied as recorded, which is the
/// entire reason old variants are kept.
#[test]
fn an_unimplemented_semantics_version_is_refused_by_name() {
    let err = Filter::parse("semantics=99;prefix=vendor/linux;elide=unchanged;trivial-merges=drop")
        .expect_err("version 99 does not exist");
    let message = format!("{err}");
    assert!(
        message.contains("99") && message.contains('1'),
        "the error must name both the recorded version and what this build knows, got {message:?}"
    );

    // And the version this build does implement is not refused.
    Filter::parse("semantics=1;prefix=vendor/linux;elide=unchanged;trivial-merges=drop")
        .expect("version 1 is implemented");
}

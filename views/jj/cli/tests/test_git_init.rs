...
#[test]
fn test_git_init_colocated_preserves_sparse_checkout() -> TestResult {
    let test_env = TestEnvironment::default();
    test_env.add_config("git.colocate = true");
    let remote_path = test_env.env_root().join("remote.git");
    let remote = git::init_bare(&remote_path);

    let kept_blob = remote.write_blob(b"kept\n")?.detach();
    let omitted_blob = remote.write_blob(b"omitted\n")?.detach();
    let mut views_tree = remote.edit_tree(gix::ObjectId::empty_tree(remote.object_hash()))?;
    views_tree.upsert(
        "omitted.txt",
        gix::object::tree::EntryKind::Blob,
        omitted_blob,
    )?;
    let views_tree_id = views_tree.write()?.detach();
    let mut root_tree = remote.edit_tree(gix::ObjectId::empty_tree(remote.object_hash()))?;
    root_tree.upsert("kept.txt", gix::object::tree::EntryKind::Blob, kept_blob)?;
    root_tree.upsert("views", gix::object::tree::EntryKind::Tree, views_tree_id)?;
    let root_tree_id = root_tree.write()?.detach();
    git::write_commit(&remote, "refs/heads/main", root_tree_id, "initial", &[]);
    git::set_symbolic_reference(&remote, "HEAD", "refs/heads/main");

    run_git(&remote_path, ["config", "uploadpack.allowFilter", "true"]);
    let work_dir = test_env.work_dir("repo");
    run_git(
        test_env.env_root(),
        [
    ...
    run_git(work_dir.root(), ["sparse-checkout", "init", "--no-cone"]);
    run_git(
        work_dir.root(),
        ["sparse-checkout", "set", "--no-cone", "/*", "!/views/"],
    );
    run_git(work_dir.root(), ["checkout", "main"]);

    assert!(work_dir.root().join("kept.txt").is_file());
    assert!(!work_dir.root().join("views/omitted.txt").exists());
    assert!(git_object_is_present_without_fetch(
        &remote_path,
        omitted_blob
    ));
    assert!(!git_object_is_present_without_fetch(
    ...
    assert_eq!(git_sparse_paths(work_dir.root()), ["views/omitted.txt"]);
    ...
    Working copy  (@) : sqpuoqvx b722db1c (empty) (no description set)
    Parent commit (@-): mwzqnpnk 8d36d0f9 main main@origin | initial
    [EOF]
    ");
    assert!(work_dir.root().join("kept.txt").is_file());
    assert!(!work_dir.root().join("views/omitted.txt").exists());
...
fn git_sparse_paths(repo_path: &Path) -> Vec<String> {
...
fn git_object_is_present_without_fetch(repo_path: &Path, object_id: gix::ObjectId) -> bool {
    Command::new("git")
        .current_dir(repo_path)
        .env("GIT_NO_LAZY_FETCH", "1")
        .args(["cat-file", "-e", &object_id.to_string()])
        .output()
        .expect("git cat-file should start")
        .status
        .success()
}

...
/// Runs `git` in `work_dir` through `jj util exec`, with the identity and the
/// `file://` permission the submodule fixtures below need.
fn run_git(work_dir: &TestWorkDir, args: &[&str]) -> CommandOutput {
    let mut jj_args = vec![
        "util",
        "exec",
        "--",
    ...
    jj_args.extend_from_slice(args);
    work_dir.run_jj(jj_args)
}

/// Creates a repo to be used as a submodule, with the Git branches `zero`,
/// `one` and `two` pointing at three different commits.
fn set_up_submodule_source(test_env: &TestEnvironment) {
    test_env
        .run_jj_in(".", ["git", "init", "--colocate", "submodule"])
        .success();
    let sub_dir = test_env.work_dir("submodule");
    for name in ["zero", "one", "two"] {
        sub_dir.write_file("file", format!("{name}\n"));
        sub_dir.run_jj(["commit", "-m", name]).success();
        // Colocation exports the bookmark as a Git branch, which is what the
        // outer repo checks out below.
        sub_dir
            .run_jj(["bookmark", "create", name, "-r", "@-"])
            .success();
    }
}

// The merge is deliberately not the working-copy commit: snapshotting drops the
// submodule from the working-copy commit, which would hide the conflict.
const MERGE_REV: &str = r#"description(substring:"merged")"#;

/// A merge of two commits that record different commits for the same Git
/// submodule. jj cannot merge these and no merge tool can either, so both
/// `jj resolve --list` and `jj resolve` must say so instead of pointing at a
/// tool that will fail.
#[test]
fn test_conflict_between_submodule_commits() {
    let test_env = TestEnvironment::default();
    set_up_submodule_source(&test_env);
    ...
    test_env
        ...
        .success();
    let work_dir = test_env.work_dir("repo");
    // The submodule has to be added and moved with git; jj has no command that
    // writes a gitlink.
    run_git(&work_dir, &["checkout", "-q", "-b", "base"]).success();
    run_git(
        &work_dir,
        &["submodule", "add", "-q", &submodule_url, "sub"],
    )
    .success();
    run_git(&work_dir, &["-C", "sub", "checkout", "-q", "zero"]).success();
    run_git(&work_dir, &["add", ".gitmodules", "sub"]).success();
    run_git(&work_dir, &["commit", "-q", "-m", "add submodule"]).success();
    run_git(&work_dir, &["checkout", "-q", "-b", "side-a"]).success();
    run_git(&work_dir, &["-C", "sub", "checkout", "-q", "one"]).success();
    run_git(&work_dir, &["add", "sub"]).success();
    run_git(&work_dir, &["commit", "-q", "-m", "side-a"]).success();
    run_git(&work_dir, &["checkout", "-q", "base"]).success();
    run_git(&work_dir, &["checkout", "-q", "-b", "side-b"]).success();
    run_git(&work_dir, &["-C", "sub", "checkout", "-q", "two"]).success();
    run_git(&work_dir, &["add", "sub"]).success();
    run_git(&work_dir, &["commit", "-q", "-m", "side-b"]).success();

    work_dir
        .run_jj(["new", "side-a", "side-b", "--no-edit", "-m", "merged"])
        .success();

    insta::assert_snapshot!(work_dir.run_jj(["resolve", "--list", "-r", MERGE_REV]), @r#"
    sub    2-sided conflict on a git submodule
    [EOF]
    ------- stderr -------
    Hint: The conflict at "sub" is between Git submodule commits, which no merge tool can resolve. jj records only the submodule's commit id, never its contents, so there is nothing for a merge tool to merge. Pick the side you want with `jj restore --from <revision> "sub"`.
    [EOF]
    "#);
    insta::assert_snapshot!(work_dir.run_jj(["resolve", "-r", MERGE_REV]), @r#"
    ------- stderr -------
    Error: The conflict at "sub" is between Git submodule commits, which no merge tool can resolve
    Hint: jj records only the submodule's commit id, never its contents, so there is nothing for a merge tool to merge. Pick the side you want with `jj restore --from <revision> "sub"`.
    [EOF]
    [exit status: 1]
    "#);
}

/// A path that is a Git submodule on one side of the conflict and a regular
/// file on the other. This is not the same as a conflict between two submodule
/// commits, and it must not be described as one.
#[test]
fn test_conflict_between_submodule_and_file() {
    let test_env = TestEnvironment::default();
    set_up_submodule_source(&test_env);
    let submodule_url = format!("{}/submodule", test_env.env_root().display());
    ...
    let work_dir = test_env.work_dir("repo");
    run_git(&work_dir, &["checkout", "-q", "-b", "base"]).success();
    work_dir.write_file("unrelated", "base\n");
    run_git(&work_dir, &["add", "unrelated"]).success();
    run_git(&work_dir, &["commit", "-q", "-m", "base"]).success();
    // The file side first, so that `sub` is free when the submodule is added.
    run_git(&work_dir, &["checkout", "-q", "-b", "side-file"]).success();
    work_dir.write_file("sub", "a regular file\n");
    run_git(&work_dir, &["add", "sub"]).success();
    run_git(&work_dir, &["commit", "-q", "-m", "side-file"]).success();
    run_git(&work_dir, &["checkout", "-q", "base"]).success();
    run_git(&work_dir, &["checkout", "-q", "-b", "side-submodule"]).success();
    run_git(
        &work_dir,
        &["submodule", "add", "-q", &submodule_url, "sub"],
    )
    .success();
    run_git(&work_dir, &["add", ".gitmodules", "sub"]).success();
    run_git(&work_dir, &["commit", "-q", "-m", "side-submodule"]).success();
    ...
    insta::assert_snapshot!(work_dir.run_jj(["resolve", "--list", "-r", MERGE_REV]), @r#"
    sub    2-sided conflict including a git submodule
    [EOF]
    ------- stderr -------
    Hint: The path "sub" is a Git submodule on some sides of the conflict and not on others, which no merge tool can resolve. jj records only the submodule's commit id, never its contents, so there is nothing for a merge tool to merge. Pick the side you want with `jj restore --from <revision> "sub"`.
    [EOF]
    "#);
    insta::assert_snapshot!(work_dir.run_jj(["resolve", "-r", MERGE_REV]), @r#"
    ------- stderr -------
    Error: The path "sub" is a Git submodule on some sides of the conflict and not on others, which no merge tool can resolve
    Hint: jj records only the submodule's commit id, never its contents, so there is nothing for a merge tool to merge. Pick the side you want with `jj restore --from <revision> "sub"`.
    [EOF]
    [exit status: 1]
    "#);

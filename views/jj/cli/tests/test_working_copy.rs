...
#[test]
fn test_submodule_not_in_store() {
    // A gitlink whose submodule jj has no repo for is the normal case, not an
    // error: `git submodule add` cloned it, jj did not, so there is nothing in
    // jj's submodule store to populate the directory from.
    let test_env = TestEnvironment::default();
    ...
    // Checking the submodule out again leaves git's clone of it alone, and says
    // so through the normal command output. The commit was created by git, so
    // its ids differ between runs and the summary lines quoting them are
    // dropped.
    let output = work_dir
        .run_jj(["edit", r#"subject("Add submodule")"#])
        .normalize_stderr_with(|stderr| {
            let mut normalized = String::new();
            for line in stderr.lines().filter(|line| {
                !line.starts_with("Working copy ") && !line.starts_with("Parent commit ")
            }) {
    ...
    // Being part of the command output, the message obeys --quiet.
    work_dir.run_jj(["new", "root()"]).success();
    let output = work_dir.run_jj(["edit", "--quiet", r#"subject("Add submodule")"#]);
    insta::assert_snapshot!(output, @"");

    // With git's clone of the submodule gone, jj creates the directory and
    // leaves it empty, because its own submodule store has no repo to populate
    // it from. This is the case that has to not crash: the design says jj
    // should fall back and say so rather than refuse to check anything out.
    work_dir.run_jj(["new", "root()"]).success();
    std::fs::remove_dir_all(work_dir.root().join("sub")).unwrap();
    let output = work_dir
        .run_jj(["edit", r#"subject("Add submodule")"#])
        .normalize_stderr_with(|stderr| {
            let mut normalized = String::new();
    ...
    These Git submodules have not been cloned, so their directories are empty:
      sub
    Hint: Clone them with `jj git submodule clone`.
    [EOF]
    ");
    assert!(work_dir.root().join("sub").is_dir());
    assert_eq!(
        std::fs::read_dir(work_dir.root().join("sub"))
            .unwrap()
            .count(),
        0
    );

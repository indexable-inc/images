...
pub async fn cmd_workspace_add(
    ...
    // A new workspace's working copy always has to be materialized, so check
    // that we are allowed to write to it before anything is created. The check
    // used to happen implicitly at the first working-copy mutation, which is
    // after `Workspace::init_workspace_with_existing_repo()` has already
    // committed the new workspace with its working-copy commit on the root
    // commit, and before the transaction below checks out `--revision`. A
    // failure there left a usable but silently mis-based workspace behind.
    command.check_new_working_copy_writable()?;

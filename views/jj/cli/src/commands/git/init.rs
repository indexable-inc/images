...
async fn do_init(
    ...
    match &init_mode {
        ...
        GitInitMode::External(git_repo_path) => {
            ...
            import_git_sparse_checkout(&mut workspace_command, &git_repo).await?;
            maybe_add_gitignore(&workspace_command)?;
...
async fn import_git_sparse_checkout(
    workspace_command: &mut WorkspaceCommandHelper,
    git_repo: &gix::Repository,
) -> Result<(), CommandError> {
    let Some(patterns) = sparse_patterns_from_git_index(git_repo)? else {
        return Ok(());
    };
    let (mut locked_ws, _) = workspace_command.start_working_copy_mutation().await?;
    locked_ws
        .locked_wc()
        .set_sparse_patterns(patterns)
        .await
        .map_err(|err| internal_error_with_message("Failed to import Git sparse checkout", err))?;
    let operation_id = locked_ws.locked_wc().old_operation_id().clone();
    locked_ws.finish(operation_id).await?;
    Ok(())
}

fn sparse_patterns_from_git_index(
    git_repo: &gix::Repository,
) -> Result<Option<Vec<RepoPathBuf>>, CommandError> {
    let Some(index) = git_repo
        .try_index()
        .map_err(|err| internal_error_with_message("Failed to read Git index", err))?
    else {
        return Ok(None);
    };
    let entries = index
        ...
        .map(|entry| {
            let path = std::str::from_utf8(entry.path(&index))
                .map_err(|err| internal_error_with_message("Invalid UTF-8 path in Git index", err))?
                .to_owned();
            let skipped = entry
                .flags
    ...
    if !entries.iter().any(|(_, skipped)| *skipped) {
        return Ok(None);
    }

    // jj has positive sparse prefixes. Block every ancestor containing a
    // skipped entry so a broad prefix cannot rematerialize that entry.
    let blocked_prefixes = entries
        .iter()
        .filter(|(_, skipped)| *skipped)
        .flat_map(|(path, _)| path_prefixes(path))
        .collect::<BTreeSet<_>>();
    let patterns = entries
        .iter()
        .filter(|(_, skipped)| !*skipped)
        .filter_map(|(path, _)| {
            path_prefixes(path).find(|prefix| !blocked_prefixes.contains(prefix))
        })
        .map(RepoPathBuf::from_internal_string)
        .collect::<Result<BTreeSet<_>, _>>()
        .map_err(|err| internal_error_with_message("Invalid path in Git index", err))?
        .into_iter()
        .collect();
    Ok(Some(patterns))
}

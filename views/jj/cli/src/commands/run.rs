...
impl WorkspacePool {
    ...
    async fn acquire(
        ...
        // `jj run` checks out into a throwaway directory, so a Git submodule
        // there gets an empty directory: there is nothing a command run over
        // one revision could do with its contents that would be recorded.
        tree_state
            .check_out(&commit.tree(), &CheckoutOptions::default())
        ...
        if is_reused_workspace {
            ...
            if !added_paths.is_empty() {
                tree_state
                    .check_out(&original_tree, &CheckoutOptions::default())
                    .map_err(|_| RunError::FailedCheckout(commit.id().clone()))?;
            }
        }

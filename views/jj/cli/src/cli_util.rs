...
/// How a stale working copy recovery was initiated.
enum StaleRecoveryMode {
    /// Explicit `jj workspace update-stale`: allowed to create a recovery
    /// commit if the working copy's operation no longer exists.
    Explicit,
    /// Implicit auto-update at command start (`snapshot.auto-update-stale`):
    /// the lost-operation case is ambiguous, so it stops with the original
    /// stale working-copy error instead of creating a recovery commit.
    Implicit { stale_err: Box<CommandError> },
}

/// The error to report when a command needs to write to a working copy but the
/// global arguments forbid it. The hint names the argument that is in the way.
fn working_copy_not_writable_error(global_args: &GlobalArgs) -> CommandError {
    let hint = if global_args.ignore_working_copy {
        "Don't use --ignore-working-copy."
    } else if global_args.no_integrate_operation {
        "Don't use --no-integrate-operation."
    } else {
        "Don't use --at-op."
    };
    user_error("This command must be able to update the working copy.").hinted(hint)
}
...
impl CommandHelper {
    ...
    pub async fn workspace_helper_with_stats(
            ...
            .await
        {
            ...
            Err(SnapshotWorkingCopyError::StaleWorkingCopy(err)) => {
                ...
                let (workspace_command, stats) = self
                    .recover_stale_working_copy_impl(
                        ui,
                        workspace,
                        env,
                        &git_import_export_lock,
                        StaleRecoveryMode::Implicit {
                            stale_err: Box::new(err),
                        },
                    )
                    .await?;
                let stale_op = old_repo.operation();
                let op_summary = match stale_op.metadata().description.lines().next() {
                    Some(desc) if !desc.is_empty() => {
                        format!("{} ({desc})", short_operation_hash(stale_op.id()))
                    }
                    _ => short_operation_hash(stale_op.id()),
                };
                writeln!(
                    ui.status(),
                    "Working copy updated: @ was rewritten by operation {op_summary}"
                )?;
    ...
    pub async fn recover_stale_working_copy(
        ...
        self.recover_stale_working_copy_impl(
            ui,
            workspace,
            env,
            &git_import_export_lock,
            StaleRecoveryMode::Explicit,
        )
        .await
    }
    ...
    async fn recover_stale_working_copy_impl(
        ...
        match workspace.repo_loader().load_operation(op_id).await {
            ...
            Err(e @ OpStoreError::ObjectNotFound { .. }) => {
                // The operation that created @ is gone (e.g. abandoned), so
                // recovery would have to guess a parent for the working-copy
                // contents. That is genuinely ambiguous, so the implicit
                // auto-update path stops with the original stale error and
                // recovery stays behind the explicit
                // `jj workspace update-stale`.
                if let StaleRecoveryMode::Implicit { stale_err } = mode {
    ...
    /// Returns true if the working copy of a workspace *created* by this
    /// command can be updated.
    ///
    /// This is deliberately weaker than [`Self::is_working_copy_writable()`]:
    /// `--at-op` is fine here because a new workspace's working copy is
    /// created at the specified operation and so is never stale, which is why
    /// [`Self::for_workable_repo()`] doesn't check it either. The other two
    /// arguments still rule it out, so this predicate matches the
    /// [`WorkspaceCommandHelper::check_working_copy_writable()`] the new
    /// workspace will perform.
    pub fn is_new_working_copy_writable(&self) -> bool {
        !self.global_args().ignore_working_copy && self.should_commit_transaction()
    }

    /// Errors out if [`Self::is_new_working_copy_writable()`] is false.
    ///
    /// Call this *before* creating a workspace on disk. Materializing the new
    /// working copy is not optional for `jj workspace add`, so a command that
    /// may not write to it is going to fail partway through -- after the
    /// workspace has already been added to the repo, but before the requested
    /// `--revision` has been checked out, leaving a workspace whose
    /// working-copy commit sits on the root commit. Refusing up front keeps
    /// the command free of side effects when it cannot finish.
    pub fn check_new_working_copy_writable(&self) -> Result<(), CommandError> {
        if self.is_new_working_copy_writable() {
            Ok(())
        } else {
            Err(working_copy_not_writable_error(self.global_args()))
        }
    }
...
impl WorkspaceCommandHelper {
    ...
    pub fn check_working_copy_writable(&self) -> Result<(), CommandError> {
        if self.may_update_working_copy {
            ...
            Err(working_copy_not_writable_error(
                self.env.command.global_args(),
            ))
        }
    }

...
pub(crate) async fn check_out_trees(
    ...
    let check_out = |name: &str, tree| -> Result<TreeState, DiffCheckoutError> {
        ...
        // A diff editor gets the same empty submodule directory a diff of the
        // gitlink would show: it edits the superproject's files, not a
        // submodule's.
        state.check_out(tree, &CheckoutOptions::default())?;
        Ok(state)
    };

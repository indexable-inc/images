...
pub struct TestBackendFactory {
    ...
    /// Tree objects read by every backend this factory made.
    ///
    /// Shared rather than per-backend so a test can ask what an operation cost
    /// without holding the backend. Some operations are supposed to read no
    /// trees at all, and "no tree objects were read" is a property worth
    /// asserting rather than timing.
    tree_reads: Arc<AtomicUsize>,
}

impl TestBackendFactory {
    /// How many tree objects backends from this factory have read.
    pub fn tree_reads(&self) -> usize {
        self.tree_reads.load(Ordering::Relaxed)
    }
    ...
    pub fn init(&self, store_path: &Path) -> TestBackend {
        ...
        TestBackend::counting(data, self.tree_reads.clone())
    }
...
impl TestBackend {
    pub fn with_data(data: Arc<Mutex<TestBackendData>>) -> Self {
        Self::counting(data, Arc::new(AtomicUsize::new(0)))
    }

    /// Same, sharing a tree-read counter with whoever else holds it.
    pub fn counting(data: Arc<Mutex<TestBackendData>>, tree_reads: Arc<AtomicUsize>) -> Self {

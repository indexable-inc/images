...
impl TestEnvironment {
    ...
    /// How many tree objects the test backend has read in this environment.
    ///
    /// Zero for environments on other backends. Tests use it to assert that an
    /// operation's cost follows the change rather than the repository: a
    /// difference of zero across a call is a claim a timing cannot make.
    pub fn tree_reads(&self) -> usize {
        self.test_backend_factory.tree_reads()
    }

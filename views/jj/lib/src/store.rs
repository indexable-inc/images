...
impl Store {
    ...
    /// The size in bytes of a file's content, exactly as many bytes as
    /// [`Self::read_file`] will yield.
    ///
    /// Backends that can answer from metadata do; the rest fall back to
    /// reading. Either way the answer is exact, never an estimate. See
    /// [`Backend::file_size`].
    pub async fn file_size(&self, path: &RepoPath, id: &FileId) -> BackendResult<u64> {
        self.backend.file_size(path, id).await
    }

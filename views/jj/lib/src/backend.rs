...
pub trait Backend: Any + Send + Sync + Debug {
    ...
    /// The size in bytes of a file's content.
    ///
    /// This must equal exactly the number of bytes [`Self::read_file`] yields.
    /// A caller may rely on it without reading, and an approximate answer is
    /// worse than a slow one: Nix writes a file into its store using the size
    /// from `stat` before it reads the content, so an under-report silently
    /// stores truncated content under a hash of bytes that were never there
    /// (<https://github.com/NixOS/nix/issues/10667>). Never estimate here.
    ///
    /// The default streams the content and counts, which is correct for every
    /// backend but costs a full read. Override it where the size can be had
    /// more cheaply, as the Git backend does from the object header.
    async fn file_size(&self, path: &RepoPath, id: &FileId) -> BackendResult<u64> {
        ...
        loop {
            ...
            total += u64::try_from(read).expect("read length fits u64");
        }
    }

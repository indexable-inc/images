...
pub async fn git_diff_part(
    ...
    match value {
        ...
        MaterializedTreeValue::GitSubmodule(id) => {
            mode = "160000";
            hash = id.hex();
            // `git apply` reads the submodule's new commit id from the body,
            // not from the abbreviated "index" line, so the id has to be
            // spelled out in full here.
            content = FileContent {
                is_binary: false,
                contents: format!("Subproject commit {hash}\n").into(),
            };
        }

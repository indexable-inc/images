...
impl TreeState {
    ...
    pub fn set_sparse_patterns(
        ...
        // Narrowing removes files, and the only files it can remove are the ones
        // this working copy materialized. `file_states` already names that set,
        // in sorted order, so the removal below reads it instead of diffing the
        // tree against an empty one.
        //
        // The diff is what made narrowing cost the size of the repository rather
        // than the size of the change. Its matcher was
        // `DifferenceMatcher::new(&old, &new)`, whose `visit()` answers
        // `AllRecursively` for every directory the new patterns do not cover, so
        // it read one tree object per directory in the whole repository --
        // sequentially, because every backend reports `concurrency() == 1` --
        // to rediscover paths the working copy could already name. Deleting a
        // file needs no tree object at all: the path is enough.
        //
        // Read in one pass, carrying each path's type with it: `file_states()`
        // rebuilds a `FileStates` view whose `debug_assert!` re-verifies that
        // every entry is sorted, so calling it per path is quadratic in a debug
        // build -- 174k entries rescanned 174k times.
        let removed_paths: Vec<(RepoPathBuf, FileType)> = {
            let removed_matcher = DifferenceMatcher::new(&old_matcher, &new_matcher);
            self.file_states()
                .iter()
                .filter(|(path, _)| removed_matcher.matches(path))
                .map(|(path, state)| (path.to_owned(), state.file_type))
                .collect()
        };
        ...
        let added_stats = self
            ...
            .block_on()?;
        let removed_stats = self.remove_materialized_paths(&removed_paths)?;
    ...
    /// Deletes materialized files by path, reading no tree objects.
    ///
    /// `paths` must be sorted and must name files this working copy
    /// materialized, which is what [`Self::file_states`] yields. Cost is one
    /// unlink per file plus one validation per *directory*, where the general
    /// checkout path pays a full path walk per file: it resets its
    /// `prev_created_path` memo on every deletion, so consecutive files in the
    /// same directory each re-validate every component from the workspace root.
    /// Nothing else needs that memo reset, and a pure deletion pass can keep the
    /// directory it just checked.
    fn remove_materialized_paths(
        &mut self,
        paths: &[(RepoPathBuf, FileType)],
    ) -> Result<CheckoutStats, CheckoutError> {
        debug_assert!(
            paths.is_sorted_by(|(a, _), (b, _)| a < b),
            "paths must be sorted and have no duplicates"
        );
        let mut stats = CheckoutStats::default();
        let mut deleted_files: HashSet<RepoPathBuf> = HashSet::new();
        let mut changed_file_states = Vec::new();
        // Directories that held something we removed, plus their ancestors, so
        // the ones that end up empty can be pruned after the unlinks rather than
        // probed once per file.
        let mut touched_dirs: BTreeSet<RepoPathBuf> = BTreeSet::new();
        // The directory whose components were last confirmed to be real
        // directories. Paths arrive sorted, so files sharing a directory
        // validate it once between them.
        let mut validated_dir: Option<RepoPathBuf> = None;
        for (path, file_type) in paths {
            let parent = path.parent().expect("file path has a parent");
            if validated_dir.as_deref() != Some(parent) {
                if !self.parent_dirs_are_plain(parent)? {
                    // A component is a symlink or a regular file, so no tracked
                    // file of ours is reachable through it. Refuse to unlink
                    // through it rather than follow it out of the workspace.
                    changed_file_states.push((path.clone(), FileState::placeholder()));
                    stats.skipped_files += 1;
                    continue;
                }
                validated_dir = Some(parent.to_owned());
                touched_dirs.extend(parent.ancestors().map(RepoPath::to_owned));
            }
        ...
        // Deepest first, so a directory is tried only after the children that
        // might have been emptying it. `remove_dir` fails on a non-empty
        // directory, which is exactly the "somebody still lives here" test.
        for dir in touched_dirs.iter().rev() {
            if dir.is_root() {
                continue;
            }
            let disk_path = dir.to_fs_path(self.working_copy_path())?;
            // Failure means non-empty or already gone; both are fine outcomes.
            drop(fs::remove_dir(&disk_path));
        }
    ...
    /// Whether every component of `dir` is a real directory.
    ///
    /// The checkout path gets this from `create_parent_dirs`, which cannot be
    /// reused here because it also creates what is missing, and a deletion pass
    /// that creates directories on its way to emptying them is absurd.
    fn parent_dirs_are_plain(&self, dir: &RepoPath) -> Result<bool, CheckoutError> {
        for ancestor in dir.ancestors() {
            if ancestor.is_root() {
                break;
            }
            let disk_path = ancestor.to_fs_path(self.working_copy_path())?;
            match disk_path.symlink_metadata() {
                Ok(metadata) if metadata.is_dir() => {}
                // Already gone: nothing under it to unlink, and nothing unsafe
                // about that.
                Err(err) if err.kind() == io::ErrorKind::NotFound => {}

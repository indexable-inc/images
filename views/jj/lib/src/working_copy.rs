...
/// Options used when checking out a commit. Some of them may be ignored by
/// some `WorkingCopy` implementations.
#[derive(Clone, Default)]
pub struct CheckoutOptions {
    /// What to populate the working copy's Git submodule directories with.
    ///
    /// A working copy cannot resolve a gitlink on its own: the submodule's repo
    /// lives in the repo's [`SubmoduleStore`], and loading a repo out of that
    /// store needs the settings and store factories the superproject was loaded
    /// with, none of which a working copy has. So the caller resolves the
    /// gitlinks and the working copy only writes what it is handed.
    ///
    /// [`SubmoduleStore`]: crate::submodule_store::SubmoduleStore
    pub submodules: SubmoduleContents,
}

/// The contents a checkout should populate Git submodule directories with.
///
/// Entries are keyed by the path the gitlink sits at *and* the commit id it
/// names, because a checkout looks at two trees and the same path may name a
/// different submodule commit in each of them.
#[derive(Clone, Debug, Default)]
pub struct SubmoduleContents {
    sources: BTreeMap<(RepoPathBuf, CommitId), SubmoduleSource>,
}

impl SubmoduleContents {
    /// Records what the gitlink for `commit_id` at `path` resolved to.
    pub fn insert(&mut self, path: RepoPathBuf, commit_id: CommitId, source: SubmoduleSource) {
        self.sources.insert((path, commit_id), source);
    }

    /// What the gitlink for `commit_id` at `path` resolved to, if the caller
    /// resolved it at all.
    pub fn get(&self, path: &RepoPath, commit_id: &CommitId) -> Option<&SubmoduleSource> {
        // The key is owned, so a lookup allocates. A checkout touches a handful
        // of gitlinks at most, so this is cheaper than a second index.
        self.sources.get(&(path.to_owned(), commit_id.clone()))
    }

    /// Whether no gitlink was resolved.
    pub fn is_empty(&self) -> bool {
        self.sources.is_empty()
    }
}

/// What a repo's submodule store had to say about one gitlink.
#[derive(Clone, Debug)]
pub enum SubmoduleSource {
    /// The submodule commit was found, and this is the tree it names.
    Tree(MergedTree),
    /// The submodule has no repo in the store. Either it has never been cloned,
    /// or the gitlink is not declared in `.gitmodules`, in which case there is
    /// no name to store it under and no url to clone it from.
    NotCloned,
    /// The submodule has a repo in the store, but the commit the gitlink names
    /// is not in it, so it has not been fetched yet.
    CommitNotFetched,
}

/// What a checkout did with one Git submodule.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum SubmoduleCheckoutOutcome {
    /// The submodule's files were written into the working copy.
    Populated {
        /// How many files were written.
        added_files: u32,
    },
    /// The submodule has no repo in the repo's submodule store, so its
    /// directory was created and left empty. This is the normal state of a
    /// submodule nobody has asked for yet, not an error.
    NotCloned,
    /// The submodule has a repo in the store, but the commit the gitlink names
    /// has not been fetched into it, so its directory was left empty.
    CommitNotFetched,
    /// The submodule's directory holds something jj did not write, so it was
    /// left exactly as it was. Overwriting it could destroy a clone the user
    /// made themselves, or work they have not committed.
    LeftAlone,
    /// The caller resolved no contents for this gitlink, so jj did not even
    /// look for it. `LockedWorkingCopy::set_sparse_patterns` and the temporary
    /// working copies of `jj run` and the diff editors do this.
    NotResolved,
}
...
pub struct CheckoutStats {
    ...
    /// What the checkout did with each Git submodule it came across, in the
    /// order it came across them. Files written inside a submodule are not
    /// counted in `added_files`: they are not tracked paths of this repo.
    pub submodules: Vec<(RepoPathBuf, SubmoduleCheckoutOutcome)>,
    /// Git submodules whose directory couldn't be created, paired with
    /// the reason why. The error is formatted eagerly because `CheckoutStats`
    /// is cloneable and comparable, and `std::io::Error` is neither.
    pub submodule_dir_errors: Vec<(RepoPathBuf, String)>,
}

...
///
/// On fetch, the local side receives objects and resolves deltas while the
/// remote side counts and compresses. On push, the roles are reversed: the
/// local side counts, compresses, and writes objects while the remote side
/// resolves deltas. Each field holds whichever direction's phase applies, so
/// the same overall progress works for both commands.
#[derive(Clone, Debug, Default)]
pub struct GitProgress {
    /// `(frac, total)` of "Resolving deltas" (local on fetch, remote on push).
    pub deltas: (u64, u64),
    /// `(frac, total)` of "Receiving objects" (fetch) or "Writing objects"
    /// (push).
    pub objects: (u64, u64),
    /// `(frac, total)` of "Counting objects" (remote on fetch, local on push).
    pub counted_objects: (u64, u64),
    /// `(frac, total)` of "Compressing objects" (remote on fetch, local on
    /// push).
...
fn read_to_end_with_progress<R: Read>(
    ...
    loop {
        ...
        if update_progress(line, &mut progress.objects, b"Receiving objects:")
            || update_progress(line, &mut progress.objects, b"Writing objects:")
            || update_progress(line, &mut progress.deltas, b"Resolving deltas:")
            || update_progress(line, &mut progress.deltas, b"remote: Resolving deltas:")
            || update_progress(line, &mut progress.counted_objects, b"Counting objects:")
            || update_progress(
                line,
...
Done";
    ...
    #[test]
    fn test_read_to_end_with_progress_push() {
        // Captured from `git push --porcelain --progress` stderr: the local
        // side enumerates, counts, compresses, and writes, and the remote
        // side resolves deltas.
        let sample = concat!(
            "Enumerating objects: 20, done.\n",
            "Counting objects:  25% (5/20)\rCounting objects: 100% (20/20), done.\n",
            "Compressing objects:  50% (5/10)\rCompressing objects: 100% (10/10), done.\n",
            "Writing objects:  45% (9/20), 1.91 MiB | 1.91 MiB/s\r",
            "Writing objects: 100% (20/20), 5.29 MiB | 10.64 MiB/s, done.\n",
            "remote: Resolving deltas:  60% (3/5)\r",
            "remote: Resolving deltas: 100% (5/5), completed with 5 local objects.\n",
        );

        let mut callback = GitSubprocessCapture::default();
        let output = read_to_end_with_progress(&mut sample.as_bytes(), &mut callback).unwrap();
        assert_eq!(output, b"");
        // "Enumerating objects" carries a count but no total, so it passes
        // through as a sideband message rather than progress.
        assert_eq!(
            callback.local_sideband,
            ["Enumerating objects: 20, done.", "\n"]
        );
        assert_eq!(callback.remote_sideband, Vec::<BString>::new());
        assert_eq!(callback.progress.len(), 8);
        // The throughput suffix of "Writing objects" doesn't confuse the
        // object counts.
        assert_eq!(callback.progress[4].objects, (9, 20));
        let last = callback.progress.last().unwrap();
        assert_eq!(last.counted_objects, (20, 20));
        assert_eq!(last.compressed_objects, (10, 10));
        assert_eq!(last.objects, (20, 20));
        assert_eq!(last.deltas, (5, 5));
    ...
    fn test_read_progress_line() {
        ...
        assert_eq!(
            read_progress_line(b"Writing objects:  45% (9/20), 1.91 MiB | 1.91 MiB/s\r"),
            Some((9, 20))
        );
        assert_eq!(
            read_progress_line(
                b"remote: Resolving deltas: 100% (5/5), completed with 5 local objects.\n"
            ),
            Some((5, 5))
        );

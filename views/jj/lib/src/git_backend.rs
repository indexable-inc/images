...
pub struct GitBackend {
    ...
    /// Whether the backing Git repository names at least one promisor remote
    /// (`remote.<name>.promisor = true`), i.e. it is a partial clone whose
    /// missing objects are fetchable on demand. Cached because it gates every
    /// object-not-found path and cannot change while the backend is open
    /// without someone rewriting the Git config underneath us, in which case
    /// a reload is the answer.
    promisor: OnceLock<bool>,
...
impl GitBackend {
    ...
    /// Whether the repository is a partial clone with a promisor remote.
    ///
    /// This is Git's own marker for "objects may be missing locally by
    /// design": `git clone --filter=...` writes `remote.<name>.promisor =
    /// true`, and every Git command consults it before declaring an object
    /// lost. A plain repository never has it, so a missing object there stays
    /// the hard error it always was.
    fn has_promisor_remote(&self, git_repo: &gix::Repository) -> bool {
        *self.promisor.get_or_init(|| {
            ...
            config
                ...
                .flatten()
                .any(|section| {
                    section.value("promisor").is_some_and(|value| {
                        gix::config::Boolean::try_from(value).is_ok_and(|boolean| boolean.0)
                    })
                })
        })
    }

    /// Tries to make a missing object present locally by asking the `git` CLI
    /// for it, which triggers Git's own promisor machinery: `git cat-file -e`
    /// on a missing object in a partial clone fetches exactly that object from
    /// the promisor remote. Returns whether the object is now present.
    ///
    /// Delegating to the CLI rather than reimplementing the fetch keeps the
    /// semantics exactly Git's: which remote to ask, what filter to send, and
    /// whether lazy fetching is allowed at all (`GIT_NO_LAZY_FETCH`) are all
    /// decided by Git, so a repository behaves the same whether it is read
    /// through jj or through git. gix itself has no promisor support, which is
    /// why this exists.
    ///
    /// One subprocess per missing object is the honest cost of on-demand
    /// fetching; a warm object store never takes this path, and gix rescans
    /// the pack directory on a lookup miss, so the retry after a successful
    /// backfill sees the newly fetched pack without reopening the repository.
    fn backfill_from_promisor(&self, git_repo: &gix::Repository, git_id: gix::ObjectId) -> bool {
        if !self.has_promisor_remote(git_repo) {
            return false;
        }
        tracing::debug!(object = %git_id, "fetching missing object from promisor remote");
        let status = Command::new(&self.git_executable)
            .arg("--git-dir")
            .arg(git_repo.path())
            .args(["cat-file", "-e"])
            .arg(git_id.to_string())
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .status();
        match status {
            ...
            Ok(_) => {
                tracing::warn!(
                    object = %git_id,
                    "promisor remote did not supply the missing object"
                );
                false
            }
            Err(err) => {
                tracing::warn!(
                    object = %git_id,
                    error = %err,
                    "could not run git to fetch a missing object"
                );
                false
            }
        }
    ...
    /// `find_object`, but a miss in a partial clone fetches the object from
    /// the promisor remote and retries once before failing.
    fn find_object_backfilling<'a>(
        &self,
        git_repo: &'a gix::Repository,
        id: &impl ObjectId,
    ) -> BackendResult<gix::Object<'a>> {
        let git_id = validate_git_object_id(git_repo, id)?;
        match git_repo.find_object(git_id) {
            Ok(object) => Ok(object),
            Err(err)
                if matches!(err, gix::object::find::existing::Error::NotFound { .. })
                    && self.backfill_from_promisor(git_repo, git_id) =>
            {
                git_repo
                    .find_object(git_id)
    ...
    /// `find_header`, with the same promisor backfill as
    /// [`Self::find_object_backfilling`].
    fn find_header_backfilling(
        &self,
        git_repo: &gix::Repository,
        id: &impl ObjectId,
    ) -> BackendResult<gix::odb::find::Header> {
        let git_id = validate_git_object_id(git_repo, id)?;
        match git_repo.find_header(git_id) {
            Ok(header) => Ok(header),
            Err(err)
                if matches!(err, gix::object::find::existing::Error::NotFound { .. })
                    && self.backfill_from_promisor(git_repo, git_id) =>
    ...
    fn read_file_sync(&self, id: &FileId) -> BackendResult<Vec<u8>> {
        let locked_repo = self.lock_git_repo();
        let mut blob = self
            .find_object_backfilling(&locked_repo, id)?
            .try_into_blob()
...
impl Backend for GitBackend {
    ...
    async fn file_size(&self, _path: &RepoPath, id: &FileId) -> BackendResult<u64> {
        let locked_repo = self.lock_git_repo();
        let header = self.find_header_backfilling(&locked_repo, id)?;
        Ok(header.size())
    }

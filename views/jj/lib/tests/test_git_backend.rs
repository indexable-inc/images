...
/// A `blob:none` partial clone serves reads like a full clone, just lazily:
/// a blob that was never fetched is pulled from the promisor remote on first
/// read instead of surfacing as `ObjectNotFound`. This is what lets a
/// `jj fs mount` (and any other store read) run against a partial clone.
#[test]
fn test_partial_clone_backfills_missing_blobs_on_read() -> TestResult {
    if !is_external_tool_installed("git") {
        eprintln!("Skipping because git command might fail to run");
        return Ok(());
    }
    ...
    // An origin repository with one commit holding one file. The two server
    // options are what a promisor-capable server (GitHub included) offers:
    // `allowFilter` accepts the `blob:none` clone, `allowAnySHA1InWant` lets
    // the later backfill ask for a blob by id.
    let origin_path = temp_dir.path().join("origin");
    std::fs::create_dir(&origin_path)?;
    run_git(&origin_path, &["init", "-q"]);
    run_git(&origin_path, &["config", "uploadpack.allowFilter", "true"]);
    run_git(
        &origin_path,
        &["config", "uploadpack.allowAnySHA1InWant", "true"],
    );
    std::fs::write(origin_path.join("file"), b"lazily fetched content\n")?;
    run_git(&origin_path, &["add", "file"]);
    run_git(
        &origin_path,
        &[
            "-c",
    ...
    let blob_hex = run_git(&origin_path, &["rev-parse", "HEAD:file"]);

    // A partial clone that never fetched the blob. `--no-local` forces the
    // smart transport; a local-path clone would hardlink the objects over and
    // silently defeat the point of the test, which is why the blob's absence
    ...
    let partial_git_dir = temp_dir.path().join("partial").join(".git");
    let blob_oid = gix::ObjectId::from_hex(blob_hex.as_bytes()).unwrap();
    assert!(
        gix::open(&partial_git_dir)
            .unwrap()
            .find_object(blob_oid)
            .is_err(),
        "the clone was not partial; the test would prove nothing"
    );

    let store_path = temp_dir.path().join("store");
    std::fs::create_dir(&store_path)?;
    let backend = GitBackend::init_external(&settings, &store_path, &partial_git_dir).unwrap();

    // Reading the missing blob backfills it from the promisor remote.
    let file_id = jj_lib::backend::FileId::try_from_hex(&blob_hex).unwrap();
    let path = repo_path("file");
    let mut reader = backend.read_file(path, &file_id).block_on()?;
    let mut content = Vec::new();
    futures::AsyncReadExt::read_to_end(&mut reader, &mut content).block_on()?;
    assert_eq!(content, b"lazily fetched content\n");

    // The object is now local: a re-read with lazy fetching forbidden would
    // also succeed, which is what "backfilled" means.
    gix::open(&partial_git_dir)
        .unwrap()
        .find_object(blob_oid)
        .expect("the blob should be present locally after the backfill");

    // The sized read path takes the same backfill.
    assert_eq!(
        backend.file_size(path, &file_id).block_on()?,
        content.len() as u64
    );
    Ok(())
}

/// The backfill is gated on the repository actually naming a promisor remote:
/// in an ordinary repository a missing object stays the hard error it always
/// was, with no subprocess spawned to ask nobody about it.
#[test]
fn test_missing_object_without_promisor_is_still_not_found() -> TestResult {
    let test_repo = TestRepo::init_with_backend(TestRepoBackend::Git);
    let repo = &test_repo.repo;
    let bogus = jj_lib::backend::FileId::from_hex("1111111111111111111111111111111111111111");
    let result = repo
        .store()
        .read_file(repo_path("nowhere"), &bogus)
        .block_on();
    match result {
        Err(err) => assert!(
            matches!(err, jj_lib::backend::BackendError::ObjectNotFound { .. }),
            "expected ObjectNotFound, got: {err:?}"
        ),
        Ok(_) => panic!("reading a bogus id unexpectedly succeeded"),
    }
    Ok(())
}

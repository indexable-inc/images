...
no longer the current commit for that workspace), `jj` automatically updates it
at the start of the next command and prints a notice naming the operation that
rewrote it. You can opt out and be prompted to run `jj workspace update-stale`
yourself by setting:
...
The automatic update never creates a recovery commit: if the working copy's
operation no longer exists (e.g. it was abandoned), the command still stops
with an error and recovery stays behind the explicit
...
For the same reason, a view will not publish a commit that changes paths it does
not carry. Deriving filters the file list and copies the message through
unchanged, because copying it is what makes a derived commit hash the published
repository's own. A commit that spans the view and the rest of the monorepo
therefore arrives with a correct file list and a message written about work that
is not in it, which is how a note meant for one repository reaches another.
`jj views push` names the commit and the offending paths and asks you to
`jj split` it; `--allow-mixed` publishes it anyway.

`jj views check` is the same scan without the push. It lists every commit that
would publish such a message, one line per commit, as `<view> <commit>
<paths>`, and exits non-zero when it finds any, so it can gate a branch before
anything is sent. It reads objects and touches no network. By default it stops
at each view's anchor, since older history has already been published;
`--all` scans everything reachable instead.

Two things are deliberately outside both checks. The root `.jj-views.toml` is
view metadata rather than work, and adopting a view writes it in the same commit
that vendors the tree, so changing it alone never counts; a commit that changes
the manifest *and* something else outside the view is still refused. Only
commits with exactly one parent are examined, so roots and merges pass. Both are
degenerate rather than trusted: a root has no before-tree to compare against and
would name the entire repository, and a merge's tree differs from every parent
outside the prefix whenever both sides changed anything out there, so it would
name every integration merge. In both cases the content was written in
single-parent commits the scan already reaches. Scanning this rule's own home
repository shows the size of that distinction: counting roots reported 92
commits, 80 of them view lift roots whose message no human wrote, against 12
real mixed edits once roots are left out.
...
A push to a view's own default branch has to be one the published branch can
fast-forward, with one exception. A published repository can end up holding its
own copies of commits this repository already derives -- the same trees under
different commit objects, because both sides created the work independently --
and no other command can reconcile that: `jj views fetch` compares content,
finds nothing missing and reports the view up to date, while `jj views anchor`
compares hashes and fails. `--replace-drifted` replaces those copies with the
commits this repository derives. It refuses unless the published tip's tree is
exactly the derived tip's, so a branch carrying content this repository does not
produce is never overwritten by it, and it writes the replaced tip to
`refs/pins/<date>-<hash>` in the same push, because lock files elsewhere may pin
that revision.

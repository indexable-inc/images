...
pub fn module<'a>(context: &'a Context) -> Option<Module<'a>> {
    ...
    // Attempt repository path contraction (if we are in a repository)
    // Otherwise use the logical path, automatically contracting
    //
    // The VCS working directory rather than `get_repo`: the latter is gix
    // discovery, so a non-colocated jj workspace has no `.git` for it to find
    // and truncation had nothing to truncate to, leaving the absolute path.
    let vcs_root = if config.truncate_to_repo || config.repo_root_style.is_some() {
        context.get_vcs_workdir()
    } else {
        None
    };
    let dir_string = if config.truncate_to_repo {
        vcs_root
            .filter(|&root| root != home_dir.as_path())
...
mod tests {
    ...
    #[test]
    fn jj_workspace_root() -> io::Result<()> {
        let tmp_dir = TempDir::new()?;
        let repo_dir = tmp_dir.path().join("rocket-controls");
        fs::create_dir_all(repo_dir.join(".jj"))?;
    ...
    #[test]
    fn directory_in_jj_workspace_truncate_to_repo_true() -> io::Result<()> {
        let tmp_dir = TempDir::new()?;
        let repo_dir = tmp_dir.path().join("above-repo").join("rocket-controls");
        let dir = repo_dir.join("src/meters/fuel-gauge");
        fs::create_dir_all(&dir)?;
    ...
    /// A plain git checkout nested inside a jj workspace is a git repository,
    /// not part of the outer workspace: the nearer marker owns the directory.
    #[test]
    #[ignore]
    fn git_repo_nested_in_jj_workspace_prefers_the_nearer_git_root() -> io::Result<()> {
        let tmp_dir = TempDir::new()?;
        let jj_dir = tmp_dir.path().join("outer");
        let git_dir = jj_dir.join("vendor").join("rocket-controls");
        fs::create_dir_all(&git_dir)?;
        fs::create_dir_all(jj_dir.join(".jj"))?;
        init_repo(&git_dir).unwrap();

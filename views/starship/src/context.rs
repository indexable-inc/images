...
pub struct Context<'a> {
    ...
    /// Private field caching the nearest VCS working directory, jj or git.
    /// Cached because every `require_repo` custom module asks for it.
    vcs_workdir: OnceLock<Option<PathBuf>>,
...
impl<'a> Context<'a> {
    ...
    /// directory: the nearest jj workspace, else the git worktree.
    ///
    /// [`Context::get_repo`] is gix discovery, so it is git-only. In a
    /// non-colocated jj workspace -- a `.jj` with no `.git`, which is jj's
    /// default -- it errors, and every caller that only wanted "where does
    /// this checkout start" then behaves as if the user were not in a
    /// repository at all.
    pub fn get_vcs_workdir(&'a self) -> Option<&'a Path> {
        self.vcs_workdir
            .get_or_init(|| {
                // One upward walk looking for both markers, so the nearest one
                // wins without comparing two paths that came from two
                // different discoveries: this scan starts from `current_dir`,
                // while gix's `workdir` need not resolve symlinks the same
                // way, so comparing component counts would lie. `.git` is a
                // file in a linked worktree or a submodule and a directory
                // otherwise, hence both `set_files` and `set_folders`.
                let nearest = self
                    .begin_ancestor_scan()
                    .set_files(&[".git"])
                    .set_folders(&[".jj", ".git"])
                ...
                match nearest {
                    // Colocated repos land here too, and give the same answer
                    // either way: `.jj` and `.git` share the directory.
                    Some(dir) if dir.join(".jj").is_dir() => Some(dir),
                    // Git is at least as near, so gix stays the authority on
                    // where its worktree starts: it honours linked worktrees,
                    // `GIT_DIR` and ceiling directories, which a marker scan
                    // does not.
                    _ => self.get_repo().ok().and_then(|repo| repo.workdir.clone()),
                }
            })
            .as_deref()
    }

    /// Whether the current directory is inside a repository at all.
    ///
    /// A different question from [`Context::get_vcs_workdir`], which names a
    /// working directory: a bare git repository is a repository and has none.
    /// Phrased as "what [`Context::get_repo`] already accepted, plus jj" on
    /// purpose, so that widening this to jj cannot narrow it for git no matter
    /// what gix discovery does or does not open.
    pub fn in_vcs_repo(&'a self) -> bool {
        self.get_repo().is_ok() || self.get_vcs_workdir().is_some()
    }

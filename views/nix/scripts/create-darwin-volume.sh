...
# Case-sensitive by default: the Nix store expects POSIX case semantics, and
# on plain (case-insensitive) APFS Nix has to fall back to the `use-case-hack`
# rename machinery for case-colliding file names (e.g. Linux kernel headers).
# Set NIX_VOLUME_FS=APFS to get the old case-insensitive behaviour.
readonly NIX_VOLUME_FS="${NIX_VOLUME_FS:-Case-sensitive APFS}"

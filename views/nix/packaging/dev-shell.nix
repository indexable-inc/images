...
pkgs.nixComponents2.nix-util.overrideAttrs (
  ...
  in
  {
    ...
    shellHook = ''
      ...
      # Install the pre-commit git hook so unformatted commits are caught
      # locally before a push, not after a red CI run.
      # `pre-commit-hooks-install` (packaging/dev-shell.nix nativeBuildInputs,
      # wraps git-hooks.nix's installationScript) is already idempotent and
      # silent when the hook it installed previously is still current, but
      # it delegates to `pre-commit install`, which migrates an unrecognized
      # existing hook to a `.legacy` file and replaces it without asking.
      # Refuse instead: leave a foreign hook in place and say so once.
      if _nix_hooks_dir="$(git rev-parse --git-path hooks 2>/dev/null)"; then
        _nix_pre_commit_hook="$_nix_hooks_dir/pre-commit"
        if [ -e "$_nix_pre_commit_hook" ] && ! grep -q "File generated by pre-commit" "$_nix_pre_commit_hook" 2>/dev/null; then
          echo "pre-commit-hooks-install: $_nix_pre_commit_hook already exists and was not installed by pre-commit; leaving it in place. Run pre-commit-hooks-install by hand to replace it." >&2
        else
          pre-commit-hooks-install
        fi
        unset _nix_pre_commit_hook
      fi
      unset _nix_hooks_dir

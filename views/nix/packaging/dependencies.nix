  ...
  boost =
    (pkgs.boost.override {
      extraB2Args = [
        ...
        "--with-iostreams"
        "--with-url"
        # The parallel evaluator's Executor spawns workers as `boost::thread`
        # rather than `std::thread`, because Boehm GC needs the thread handle to
        # register each worker's stack as a root. Without this module libnixexpr
        # fails to link on `boost::thread::start_thread_noexcept`.
        "--with-thread"
      ];
      enableIcu = false;
    }).overrideAttrs

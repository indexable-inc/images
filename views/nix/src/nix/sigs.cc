...
#include "store-copy-sigs.md"
    ...
    void run(ref<Store> store, StorePaths && storePaths) override
    {
        ...
        /* Create the pool after everything the work items reference, in
           particular `added`, so its destructor joins the workers before
           any of that is destroyed. */
        ThreadPool pool{fileTransferSettings.httpConnections};

        for (auto & storePath : storePaths)
            if (!pool.tryEnqueue(std::bind(doPath, storePath)))
                break;

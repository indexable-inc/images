...
/* Same reasoning as `LocalStoreAccessor::getFingerprint`: every path this
   accessor serves lives inside a store object whose content is fixed once
   it is registered. */
std::pair<CanonPath, std::optional<std::string>> RemoteFSAccessor::getFingerprint(const CanonPath & path)
{

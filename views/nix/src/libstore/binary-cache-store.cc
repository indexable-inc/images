    ...
    Source & narSource, RepairFlag repair, CheckSigsFlag checkSigs, fun<ValidPathInfo(HashResult)> mkInfo)
{
    ...
    for (auto & ref : info.references)
        try {
            ...
            /* Typed rather than a bare `Error`: a caller has to be able to tell this
               apart from a credential or transport failure, because retrying cannot
               satisfy it. See `BinaryCacheMissingReference` (ENG-12418). */
            throw BinaryCacheMissingReference(
                HintFmt(

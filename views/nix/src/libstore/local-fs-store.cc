...
struct LocalStoreAccessor : PosixSourceAccessor
{
    ...
    /* Every path this accessor serves lives inside a store object, whose
       content is fixed once it is registered, so a fingerprint is
       available without reading a single file. Without this the whole
       subtree is re-hashed into the store on every evaluation that
       reaches a path under `/nix/store` (indexable-inc/index#4323). */
    std::pair<CanonPath, std::optional<std::string>> getFingerprint(const CanonPath & path) override
    {
        return store->getStoreObjectFingerprint(path);
    }
};

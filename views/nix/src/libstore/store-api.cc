            ...
            if (!isValidPath(info.path)) {
                ...
                try {
                    ...
                    if (!settings.getWorkerSettings().keepGoing)
                        /* `throw e` would copy-initialise a fresh `Error` from the
                           reference and slice the exception down to its static type,
                           so a caller of `copyPaths` cannot tell a typed store failure
                           (`BinaryCacheMissingReference`) from any other. Rethrow the
                           original object instead. ENG-12418. */
                        throw;

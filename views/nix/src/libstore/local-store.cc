...
void LocalStore::registerDrvOutput(const Realisation & info)
{
    ...
    retrySQLite<void>([&]() {
        ...
        if (auto oldR = queryRealisation_(*state, info.id)) {
            ...
            /* `RegisterRealisedOutput` fills `outputPath` from a subselect
               over `ValidPaths`. When this store has no row for the output
               path the subselect yields NULL and SQLite reports `NOT NULL
               constraint failed: Realisations.outputPath`, which names
               neither the realisation nor the path and reads like database
               corruption rather than a missing registration. Look it up and
               say what is wrong. */
            if (!isValidPath_(*state, info.outPath))
                throw Error(
                    "cannot register realisation '%s': its output path '%s' is not valid in this store",
                    info.id.to_string(),
                    printStorePath(info.outPath));

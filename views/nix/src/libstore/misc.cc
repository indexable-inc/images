...
MissingPaths Store::queryMissing(const std::vector<DerivedPath> & targets)
{
    ...
    /* Every work item captures this frame by reference, and `pool` is declared
       first, so `~ThreadPool` joins the workers only after `state_` and the
       lambdas above have already been destroyed. `processGraph()` avoids that
       by creating its pool last; here the lambdas name `pool`, so it cannot
       move. Join explicitly instead, before anything the workers point at goes
       away. */
    Finally joinWorkers([&] { pool.shutdown(); });

    for (auto & path : targets) {
        /* Stop if the thread pool is shutting down. It means a previous
           work item threw an exception, so process() below will rethrow
           it. Same reasoning as processGraph(). */
        if (!pool.tryEnqueue(std::bind(doPath, path)))
            break;
    }

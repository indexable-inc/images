...
void initLibStore(bool loadConfig)
{
    ...
    /* The case hack is only needed when the store lives on a
       case-insensitive filesystem; on a case-sensitive store (e.g. a
       "Case-sensitive APFS" volume, the current macOS installer
       default) it would needlessly mangle case-colliding names, so
       probe the store directory and default it accordingly. An
       explicit `use-case-hack` setting always wins. */
    if (auto insensitive = isCaseInsensitiveDirectory(StoreConfigBase::getDefaultNixStoreDir()))
        setDefaultUseCaseHack(*insensitive);

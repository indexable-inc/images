...
#  endif
        ...
        void failPausedStall(PausedTransferVerdict verdict) noexcept
        {
            ...
            unpause();
            const char * why =
                verdict == PausedTransferVerdict::failHalfClosed
...
/* Cause this thread to be notified on SIGINT. */
        ...
        while (!quit) {
            ...
            /* A curl_multi_wakeup() issued while this thread is outside
               curl_multi_poll can be lost: libcurl (observed with 8.21.0)
               drains the wakeup eventfd on entering curl_multi_poll before
               it blocks, so the pending wakeup no longer makes the poll
               return early. The first transfer of every process hits
               exactly that window deterministically -- enqueueItem() pushes
               the item and calls wakeupMulti() before this thread has ever
               reached curl_multi_poll -- and then waits out the whole idle
               sleep (10s with no active items) before the transfer starts.
               Re-check the queues under the state lock after computing the
               sleep and refuse to sleep when actionable work is already
               pending; embargoed retries cap the sleep at their due time
               instead. A wakeup that arrives while we are INSIDE
               curl_multi_poll still interrupts it as before, so this closes
               the outside-the-poll window without busy-waiting. */
            {
                auto state(state_.lock());
                if (state->isQuitting() || !state->unpause.empty())
                    sleepTimeMs = 0;
                else if (!state->incoming.empty() && items.size() < maxQueueSize) {
                    auto embargo = state->incoming.top()->embargo;
                    ...
                    sleepTimeMs =
                        embargo <= nowTp
                            ...
                            : std::min(
                                  (long) sleepTimeMs,
                                  (long) std::chrono::duration_cast<std::chrono::milliseconds>(embargo - nowTp).count()
                                      + 1);
                }
            }

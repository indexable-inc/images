...
#  endif
        ...
        */
        ...
        if (helper.wait() != 0) {
            /* Called only for its throw: it reads the child's `\1` error and
               rethrows it. This runs inside `startChild()`, so `startLogRelay()`
               has not run and `builderOut` does not exist yet; the setup
               messages are still on the pseudoterminal itself. */
            processSandboxSetupMessages(builderOutMaster.get());

...
/**
 * Whether this store keeps a realisation for `drv`'s outputs, and so whether
 * there is anything for publication to send alongside the NARs.
 *
 * `registerOutputs` signs and registers a realisation under exactly this
 * condition. Publication has to agree with it: publishing more would put an
 * unsigned realisation for an impure derivation on a shared cache, telling
 * every other host that a one-off result is what that derivation produces;
 * publishing less would drop the realisation of a build that has one, which is
 * the bug this predicate exists to keep fixed.
 */
static bool keepsRealisations(const BasicDerivation & drv)
{
    return experimentalFeatureSettings.isEnabled(Xp::CaDerivations) && !drv.type().isImpure();
}
...
static void publishOutputs(
    LocalStore & store, const StorePath & drvPath, const BasicDerivation & drv, const SingleDrvOutputs & builtOutputs)
{
    ...
    /* The CLOSURE of the outputs, not the bare outputs. A binary cache refuses any
       store object whose references are not already valid THERE
       (`BinaryCacheStore::addToStore`), and an output can reference a store object
       that no publication step will ever reach on its own: one added by the
       EVALUATOR -- a flake input tree, a `builtins.path` -- has no deriver, so no
       build of it ever runs and `publishOutputs` is never called for it. nixpkgs'
       `separateDebugInfo` makes exactly that a hard reference of every `-debug`
       output, by symlinking the source tree under `.build-id`. Copying the closure
       makes the destination's precondition satisfiable by construction, and orders
       references before referrers.

       This is not a per-build upload cost. `copyPaths` opens with
       `dstStore.queryValidPaths()` and uploads only what the destination lacks, so
       the steady state is the same upload set plus one query per reference. It is
       also what makes the local store safe to prune: an output whose references were
       left behind is not durable upstream however many copies of it are. ENG-12418. */
    try {
        auto dstStore = getWriteThroughStore(uri);
        copyClosure(store, *dstStore, paths, NoRepair, NoCheckSigs, NoSubstitute);

        /* And the realisations, which that copy does not carry. Only the
           `RealisedPath::Set` overload of `copyPaths` registers them, and
           routing this through it is a trap rather than a shortcut: the
           matching `copyClosure` overload computes the closure of that set's
           *opaque* members only, so a set of realisations closes over nothing
           and the outputs' references are left behind -- which is the failure
           the comment above is about. Copy the closure, then register.

           In that order, and not the other way round. A realisation is the
           thing that tells another host "this derivation's output is that
           path"; publishing it before the NAR advertises a path the
           destination cannot yet serve. Publishing it after costs, at worst, a
           rebuild by whoever asked in the window.

           Without this a floating content-addressed output arrives as a NAR
           nothing can ask for. Its path is not derivable from the derivation,
           so a host holding the derivation and no realisation has no name to
           look up and rebuilds, every time, however warm the cache is. */
        if (keepsRealisations(drv))
            for (auto & [outputName, realisation] : builtOutputs)
                dstStore->registerDrvOutput(realisation);
    } catch (BinaryCacheMissingReference & e) {
        /* Permanent, not transient. Having just published the closure, a reference the
           destination still lacks is a precondition no retry of this build can satisfy:
           the object is not somewhere else waiting to arrive, it is absent. */
        throw BuildError(
            BuildResult::Failure::PermanentFailure,
            "the outputs of '%s' were built but not published: the store '%s' rejected '%s' because its "
            "reference '%s' is not present there, and publishing the closure did not put it there. A store "
            "object added by the evaluator (a flake input tree, a `builtins.path`) has no deriver, so no "
            "build ever produces it and it reaches a cache only inside some output's closure; publish that "
            "closure, or the reference itself, before this output can be accepted.",
            store.printStorePath(drvPath),
            uri,
            store.printStorePath(e.path),
            store.printStorePath(e.reference));
    } catch (Error & e) {
        throw BuildError(
...
SingleDrvOutputs DerivationBuilderImpl::unprepareBuild()
{
    ...
    publishOutputs(store, drvPath, drv, builtOutputs);
...
SingleDrvOutputs DerivationBuilderImpl::registerOutputs()
{
    ...
    for (auto & outputName : sortedOutputNames) {
        ...
        auto newInfoFromCA = [&](const DerivationOutput::CAFloating outputHash) -> ValidPathInfo {
            ...
            /* FIXME deduplicate with addToStore */
            std::string oldHashPart{scratchPath->hashPart()};
            /* Set by the content-addressing pass below when that pass already
               serialised the output as a NAR, so that the NAR hash comes from
               the same walk rather than a second one. Stays unset when the
            ...
            if (*scratchPath != newInfo0.path) {
                ...
                /* That rewrite changed the contents, so a NAR hash taken before
                   it no longer describes them. */
                narHashAndSize.reset();
            }

            if (!narHashAndSize)
                narHashAndSize = hashPath(

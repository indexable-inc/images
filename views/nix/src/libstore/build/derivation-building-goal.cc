    ...
    PathLocks outputLocks)
{
    ...
    /* Take whatever the builder said that the poll loop above has not been
       round to collect. On the timeout path below the build is abandoned
       without ever reading to EOF, so this is the only thing that keeps the
       last words of a builder killed by `--timeout` or `--max-silent-time`. */
    auto absorbRemainingLog = [&] {
        auto rest = builder->drainRemainingLog();
        if (rest.empty())
            return;
        logSize += rest.size();
        (*buildLog)(rest);
    ...
    while (true) {
        ...
        if (auto * output = std::get_if<ChildOutput>(&event)) {
            ...
            absorbRemainingLog();
            buildLog->flush();
            /* A builder can cross the limit with bytes that only arrive in the
               absorb above, and which arm of this chain sees a spewing builder
               first is a matter of scheduling, so the limit the operator set
               answers for the failure here too. */
            if (worker.settings.maxLogSize && logSize > worker.settings.maxLogSize)
                co_return doneFailureLogTooLong(*buildLog);
            co_return doneFailure(std::move(*timeout));
        }

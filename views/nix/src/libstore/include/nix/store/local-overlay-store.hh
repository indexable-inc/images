...
private:
    ...
    /**
     * Register `path` and its closure in this store's own database if the
     * lower store has them and this store does not.
     *
     * Unlike `isValidPathUncached`, this reads the upper database directly
     * rather than through `Store::isValidPath`, so a `pathInfoCache` entry
     * seeded from the lower store cannot suppress the copy-up.
     */
    void ensureUpperValidPath(const StorePath & path);

    /**
     * First copy up any lower store realisation with the same key, so we
     * merge rather than mask it, and copy up the output paths both
     * realisations name so this store's database can key on them.
     */

...
#ifdef __APPLE__
    Setting<bool> cloneStorePaths{
        this,
        false,
        "clone-store-paths",
        R"(
          Whether the store optimiser deduplicates identical files with APFS
          copy-on-write clones (`clonefile`) instead of hard links.

          Cloned files share their data extents, so the disk saving is the
          same, but each path keeps its **own inode**. That matters on macOS:
          dyld resolves `@rpath`/`@loader_path` through the canonical path of
          the executing file (`fcntl(F_GETPATH)`), which for a hard-linked
          file can be a *sibling store path* that the build sandbox never
          allow-listed, making every invocation of the affected binary fail to
          load its libraries. Clones cannot produce that aliasing.

          Note that this is not useful together with `compress-store-paths`:
          compressing a file rewrites it and so breaks extent sharing.
        )"};

    Setting<bool> compressStorePaths{
        this,
        false,
        "compress-store-paths",
        R"(
          Whether to apply macOS transparent (decmpfs) compression, using
          LZFSE, to files added to the store. The kernel decompresses on read,
          so file contents -- and therefore NAR hashes and store paths -- are
          unaffected; only the on-disk footprint shrinks. Files that would not
          save at least one allocation block are left alone, and every
          compressed file is read back and compared before the compression is
          accepted.
        )"};
#endif

...
public:
    ...
    Setting<std::string> writeThroughStore{
        ...
        R"(
          ...
          When this is set to a store URL, every locally built derivation has the closure
          of its outputs copied to that store after the outputs are registered valid and
          before the build goal reports success, and a failure to copy fails the build.
          Publication is therefore synchronous with the build: once a build reports
          success its outputs are durable on the destination, which leaves the local
          store free to be treated as a prunable cache.

          The closure rather than the outputs, for two reasons. A binary cache refuses a
          store object whose references are not valid there, and an output can reference
          one that no build ever produces: a store object added by the evaluator (a flake
          input tree, a `builtins.path`) has no deriver, and `separateDebugInfo` makes
          exactly that a reference of every `-debug` output. And an output whose
          references were left behind is not durable upstream, so pruning the local store
          would lose them. References the destination already has are not re-uploaded.

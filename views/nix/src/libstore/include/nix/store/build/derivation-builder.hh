...
struct DerivationBuilder : RestrictionContext
{
    ...
    /**
     * The builder's standard output/error, as the scheduler sees it: the
     * read end of a pipe that a thread of ours fills from the master side
     * of the builder's pseudoterminal. EOF here is EOF on that master,
     * which is the last slave fd closing, so the scheduler's contract is
     * the same one it has always had.
     */
    AutoCloseFD builderOut;

    /**
     * Return whatever @ref builderOut still holds that the poll loop has
     * not picked up, without blocking.
     *
     * Only the abandoned paths need this: the scheduler reads to EOF on a
     * clean end of build, but a builder killed by `--timeout` or
     * `--max-silent-time` is dropped with its last words possibly still in
     * the pipe.
     */
    virtual std::string drainRemainingLog()
    {
        return {};
    }
    ...
    /** Process group leader for a running local builder, if available. */
    virtual std::optional<pid_t> getPid() = 0;
};

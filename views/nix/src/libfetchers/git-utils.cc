...
struct GitRepoImpl : GitRepo, std::enable_shared_from_this<GitRepoImpl>
{
    ...
    Hash getTreeHash(const Hash & rev) override
    {
        auto commit = peelObject<Commit>(lookupObject(*this, hashToOID(rev)).get(), GIT_OBJECT_COMMIT);

        return toHash(*git_commit_tree_id(commit.get()));
    }

    bool hasCommitExtraHeader(const Hash & rev, const std::string & field) override
    {
        auto commit = peelObject<Commit>(lookupObject(*this, hashToOID(rev)).get(), GIT_OBJECT_COMMIT);

        git_buf buf = GIT_BUF_INIT;
        if (git_commit_header_field(&buf, commit.get(), field.c_str()) != 0)
            return false;
        git_buf_dispose(&buf);
        return true;
    }

...
{
    /* SetUp() GTEST_SKIPs before ever reaching the `#ifndef _WIN32`
       block that assigns serverPid, so on Windows it is still
       default-constructed here. Guard with the same `#ifndef _WIN32`
       SetUp() uses around the assignment, rather than a runtime `!=
       -1` check: `Pid` has no portable "is this set" query, only a
       POSIX-only `operator pid_t()`, so a runtime check here would not
       compile for Windows at all. Without this, a skipped test fails
       its teardown instead of being cleanly skipped. */
#ifndef _WIN32
    serverPid.kill();
#endif
    delTmpDir.reset();

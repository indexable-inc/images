...
#  endif
    ...
    /* LOCAL_PEERCRED (xucred) carries no pid; macOS and FreeBSD expose the
       peer's pid through a separate sockopt. Without it the daemon records
       no clientPid, so per-client build attribution (and the disconnect
       liveness monitor keyed on it) silently degrades on these platforms. */

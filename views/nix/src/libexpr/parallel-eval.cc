...
{
    /* Multi-threaded evaluation is opt-in twice over: the experimental feature
       admits it at all, and `eval-cores` picks the width. Refuse rather than
       silently falling back to one core, so a caller that asked for threads
       without the feature hears about it. */
    if (evalSettings.evalCores != 1U)
        experimentalFeatureSettings.require(Xp::ParallelEval);

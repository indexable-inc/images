    ...
    EvalState & state, bool strict, Value & v, const PosIdx pos, NixStringContext & context, bool copyToStore)
{
    ...
    auto recurse = [&](this const auto & recurse, json & res, Value & v, PosIdx pos) -> void {
        ...
        case nString: {
            /* toJSON is how rendered configuration reaches a derivation
               here; it reads strings without going through forceString, so
               without this hook a value that flows into a JSON document
               loses its provenance exactly where it matters most. */
            if (state.readSetTracker) [[unlikely]]
                state.readSetTracker->provConsume(v);
        ...
        case nPath:
            if (copyToStore)
                res = state.store->printStorePath(state.copyPathToStore(context, v.path()));
            else
                res = v.path().path.abs();
            break;

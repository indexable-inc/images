    ...
    EvalState & state, PosIdx pos)
{
    ...
    if (pd == pdThunk) {
        ...
        // Set once this thread owns the thunk, so the failure path can
        // reconstruct it as a recovery value.
        Env * thunkEnv = nullptr;
        Expr * thunkExpr = nullptr;
        Value * appLeft = nullptr;
        Value * appRight = nullptr;
        ...
        try {
            ...
            if (p1_ & discriminatorMask) {
                appLeft = untagPointer<Value *>(p0_);
                appRight = untagPointer<Value *>(p1_);
                state.callFunction(*appLeft, *appRight, (Value &) *this, pos);
            } else {
                thunkEnv = untagPointer<Env *>(p0_);
                thunkExpr = untagPointer<Expr *>(p1_);
                thunkExpr->eval(state, *thunkEnv, (Value &) *this);
            }
        } catch (...) {
            state.handleEvalException((Value &) *this, pos, thunkEnv, thunkExpr, appLeft, appRight);
            throw;
        }
    }
    ...
    if (InternalType(p0_ & 0xff) == tFailed) [[unlikely]] {
        auto * failed = std::bit_cast<Failed *>(p1);
        if (failed->recoveryValue)
            state.retryFailed((Value &) *this, pos);
        else
            failed->rethrow();
    }
}

...
public:
    ...
    /**
     * Place the error at a position that is not in the `PosTable`.
     *
     * The `PosIdx` overload above can only name somewhere cppnix's own parser
     * has been, which is every position cppnix itself raises. An alternative
     * evaluator computes positions from its own tables and has none to offer,
     * so without this it can produce a correct line and column and have no
     * way to attach them -- `err` is protected, and building the error from
     * an `ErrorInfo` instead loses `debugThrow`.
     */
    [[nodiscard, gnu::noinline]] EvalErrorBuilder<T> & atPos(std::shared_ptr<const Pos> pos);

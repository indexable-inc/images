...
struct ValueBase
{
    ...
    struct Failed : gc
    {
        ...
        /**
         * Thunk to evaluate instead of rethrowing `ex` on the next
         * force. Set iff `ex` is a `RecoverableEvalError`.
         */
        Value * recoveryValue = nullptr;
...
public:
     ...
     *
     * @param invalidIsThunk Instead of UB on an uninitialized internal
     * type, return `nThunk`.
     */
    ...
    inline ValueType type() const
    {
        ...
        /* Every other discriminator comes out of the alignment bits, so
           this is the only index the table cannot answer for. */
        if (it == tUninitialized) [[unlikely]] {
    ...
    inline void mkFailed(std::exception_ptr ex, Value * recovery) noexcept
    {
        setStorage(new Value::Failed{.ex = ex, .recoveryValue = recovery});
    }

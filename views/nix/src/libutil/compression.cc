...
#undef NIX_DEF_LA_ALGO_CASE
        ...
        if (parallel)
            // `threads == 0` asks libarchive for its own default, which is
            // "as many threads as the hardware has" (the only option before
            // this parameter existed). A filter that does not implement a
            // `threads` option (anything but xz and zstd) rejects this with
            // ARCHIVE_WARN or worse, which `check` turns into a thrown
            // error naming the filter, not a silently ignored request.
            check(
                archive_write_set_filter_option(archive, format, "threads", std::to_string(threads).c_str()),
                std::string("compression method '") + format + "' does not support multi-threaded compression (%s)");
...
ref<CompressionSink>
makeCompressionSink(CompressionAlgo method, Sink & nextSink, const bool parallel, int level, unsigned int threads)
{
    switch (method) {
    case CompressionAlgo::none:
...
std::string compress(CompressionAlgo method, std::string_view in, const bool parallel, int level, unsigned int threads)
{
    StringSink ssink;

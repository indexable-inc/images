...
public:
     ...
     *
     * Throws ThreadPoolShutDown if the pool is shutting down because a
     * work item threw.
     */
    void enqueue(work_t t);

    /**
     * Like enqueue(), but return false, discarding the item, instead of
     * throwing when the pool is shutting down.
     *
     * Use this to feed the pool from the thread that will call
     * process(). With the throwing form, ThreadPoolShutDown unwinds the
     * feeding frame while workers may still be running and referencing
     * objects in it (a use after free, see Store::queryMissing), and it
     * replaces the work item's real exception, which process() rethrows,
     * with a generic shutdown error.
     */
    bool tryEnqueue(work_t t);
...
void processGraph(const std::set<T> & nodes, fun<std::set<T>(const T &)> getEdges, fun<void(const T &)> processNode)
{
    ...
    for (auto & node : nodes) {
        ...
        if (!pool.tryEnqueue(std::bind(worker, std::ref(node))))
            break;
    }

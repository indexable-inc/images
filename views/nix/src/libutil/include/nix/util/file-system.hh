...
/**
 * Best-effort probe of whether the filesystem holding the directory
 * `dir` treats file names case-insensitively. Zero-write: picks an
 * entry of `dir` whose name contains an ASCII letter and looks up the
 * case-flipped name; if that resolves to the same inode, the
 * filesystem is case-insensitive. Returns `std::nullopt` when it
 * cannot tell (missing or empty directory, no entry with a letter in
 * its name, or I/O errors).
 */

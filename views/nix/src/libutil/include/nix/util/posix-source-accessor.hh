...
public:
    ...
    /**
     * Drop the process-global lstat cache.
     *
     * That cache is keyed by absolute path and lives as long as the
     * process, which is sound for an immutable path and wrong for a
     * working tree an evaluator revisits: the second evaluation reads the
     * first one's mtimes, so a touched file reports as unchanged and a
     * `path:` input keeps its stale `lastModified`. An evaluator that
     * outlives one evaluation has to call this between evaluations.
     * Compare `GitRepo::clearCachedWorkdirInfo`, the same mitigation for
     * the git workdir cache.
     */

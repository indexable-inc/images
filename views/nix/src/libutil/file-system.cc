...
std::optional<bool> isCaseInsensitiveDirectory(const std::filesystem::path & dir)
{
    try {
        for (auto & entry : DirectoryIterator{dir}) {
            ...
            if (flipped == name)
                /* No ASCII letters to flip; try another entry. */
                continue;
            auto st1 = maybeLstat(entry.path());
            if (!st1)
                continue;
            auto st2 = maybeLstat(dir / flipped);
            if (!st2)
                /* The case-flipped name does not resolve, so lookups
                   are case-sensitive. */
                return false;
            /* If the flipped name resolves to the very same inode, the
               filesystem folded the case for us. (If it resolves to a
               *different* inode, both spellings exist as distinct
               files, which itself proves case-sensitivity.) */
            return st1->st_dev == st2->st_dev && st1->st_ino == st2->st_ino;
        }
        /* No usable entry (e.g. a fresh, still-empty store directory).
           Fall back to case-flipping the directory's own last
           component; the lookup happens in the parent directory, which
           almost always lives on the same filesystem (e.g. `/nix` for
           `/nix/store`). */
        ...
        if (flipped != name) {
            auto st1 = maybeLstat(dir);
            auto st2 = maybeLstat(dir.parent_path() / flipped);
            if (st1 && !st2)
                return false;
            if (st1 && st2)
                return st1->st_dev == st2->st_dev && st1->st_ino == st2->st_ino;

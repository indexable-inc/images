    ...
    while (!todo.empty()) {
        ...
        if (c == "" || c == ".")
            ...
            if (mode == SymlinkResolution::Full || !todo.empty()) {
                recordSourceRead(*this, res, SourceReadKind::metadata);
                auto st = maybeLstat(res);
                /* The answer, not only the question. Symlink resolution stats
                   every component of every path it is given, so leaving these
                   without an observed value leaves a large share of a read set
                   undecidable: 1 of the 3 kinds a small evaluation records. */
                recordSourceObserved(
                    *this, res, SourceReadKind::metadata, st ? SourceAccessor::Stat(*st).typeString() : "absent");
                if (st && st->type == SourceAccessor::tSymlink) {
                    if (!linksAllowed--)
                        throw Error("infinite symlink recursion in path '%s'", showPath(path));
                    recordSourceRead(*this, res, SourceReadKind::link);
                    auto target = readLink(res);
                    recordSourceObserved(*this, res, SourceReadKind::link, target);

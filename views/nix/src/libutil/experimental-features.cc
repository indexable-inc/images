...
constexpr std::array<ExperimentalFeatureDetails, numXpFeatures> xpFeatureDetails = {{
    ...
    {
        .tag = Xp::GitExportHistory,
        .name = "git-export-history",
        .description = R"(
            Allows Git fetcher inputs (`builtins.fetchGit`, `fetchTree` and
            flake inputs of type `git`) to opt into exporting structured
            commit history via the `exportHistory` and `historyDepth`
            attributes. The resulting source attribute set gains a `history`
            attribute: a list of commits (newest first, deterministic
            topological order) with revision, parents, author, committer,
            message and the paths touched by each commit.
        )",
        .trackingUrl = "https://github.com/NixOS/nix/milestone/60",
    },
    {
    ...
    {
        .tag = Xp::InvocationRecords,
        .name = "invocation-records",
        .description = R"(
            Makes every `nix` command record what it did to
            `$XDG_STATE_HOME/nix/invocations/<invocation-id>/`, and print that
            invocation id when it exits. The record holds the timestamped
            internal-json event stream, the evaluator statistics, and the
            command line, so
            [`nix invocation show`](@docroot@/command-ref/new-cli/nix3-invocation-show.md)
            can report evaluation cost, per-derivation build durations and the
            machine each derivation was built on after the command has
            finished.
        )",
        .trackingUrl = "https://github.com/NixOS/nix/milestone/60",
    },

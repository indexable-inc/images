...
#endif
        "use-case-hack",
        "Whether to enable a hack for dealing with file name case collisions on "
        "case-insensitive store filesystems (historically the macOS default). "
        "When the store directory is detected to live on a case-sensitive "
        "filesystem, this defaults to false."};
};
...
{
    archiveSettings.useCaseHack.setDefault(enable);
}
...
std::optional<std::string> stripCaseHackSuffix(std::string_view name)
{
    if (!archiveSettings.useCaseHack)
        return std::nullopt;
    auto pos = name.find(caseHackSuffix);

    ...
    case SourceAccessor::tDirectory: {
        ...
        for (auto & [name, _] : path.readDirectory()) {
            ...
            /* If we're on a case-insensitive system like macOS, undo
               the case hack applied by restorePath(). Without this, a
               tree restored from a NAR and re-hashed with the git
               method serializes the hacked names and lands on a hash
               no other system agrees with -- the NAR dump strips the
               suffix at the same point (archive.cc). */
            auto name2 = stripCaseHackSuffix(name).value_or(std::string(name));

            if (entry.mode == Mode::Directory)
                name2 += "/";

            if (!entries.emplace(name2, std::move(entry)).second)
                throw Error("file name collision between '%s' and '%s' in '%s'", name, name2, path);
        }

...
struct JSONLogger : Logger
{
    ...
    /**
     * Stamp every record with `us`, microseconds since the Unix epoch. Off by
     * default because the stream is a stable output format that golden tests
     * and third-party parsers compare against; a reader that wants to measure
     * durations after the fact (see `nix invocation show`) asks for it.
     */
    bool timestamps;

    JSONLogger(Descriptor fd, bool includeNixPrefix, bool timestamps = false)
        : fd(fd)
    ...
    void stamp(nlohmann::json & json)
    {
        if (timestamps)
            json["us"] = std::chrono::duration_cast<std::chrono::microseconds>(
...
std::unique_ptr<Logger> makeJSONLogger(Descriptor fd, bool includeNixPrefix, bool timestamps)
{
    return std::make_unique<JSONLogger>(fd, includeNixPrefix, timestamps);
}

std::unique_ptr<Logger> makeJSONLogger(const std::filesystem::path & path, bool includeNixPrefix, bool timestamps)
{
    struct JSONFileLogger : JSONLogger
    {
        AutoCloseFD fd;

        JSONFileLogger(AutoCloseFD && fd, bool includeNixPrefix, bool timestamps)
            : JSONLogger(fd.get(), includeNixPrefix, timestamps)
            , fd(std::move(fd))
        {
        }
    };
    ...
    return std::make_unique<JSONFileLogger>(std::move(fd), includeNixPrefix, timestamps);
}

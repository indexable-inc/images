...
{
    /* `pid == -1` is the default-constructed and the already-waited-on
       state, both of which are ordinary conditions a caller can be
       holding, not an invariant this class can enforce on its own (a
       default-constructed `Pid` is exactly this state, and nothing
       stops a caller from keeping one around unassigned). Both
       interpretations the syscall below would give -1 are dangerous:
       passed as `-pid` (the `separatePG` branch) it becomes `kill(1,
       ...)`, PID 1, the init process; passed as `pid` directly it is
       POSIX's own `kill(-1, ...)`, a broadcast to every process the
       caller may signal. So this has to be checked before the syscall,
       not delegated to `wait()` below.  */
    if (pid == -1)
        throw Error("Pid::kill() called with no child process to kill");
...
{
    /* See the comment in `kill()` above: a default-constructed or
       already-waited-on `Pid` is a state a caller can be holding, and
       `waitpid(-1, ...)` (POSIX: wait for any child of the calling
       process) is not what a caller asking to wait on a specific,
       unset `Pid` means, so this cannot be left to fail some other
       way inside the loop below. */
    if (pid == -1)
        throw Error("Pid::wait() called with no child process to wait for");
    while (1) {
        int status;

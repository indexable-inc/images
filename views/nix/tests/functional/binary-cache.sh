...
# Take the compressor from what the cache actually wrote rather than assuming
# the default. The replacement below has to use the same algorithm, or the
# failure is a decompression error instead of the hash mismatch being tested.
nar=$(find "$cacheDir/nar/" -type f -name "*.nar.*" | head -n1)
case "$nar" in
    *.nar.xz) narCompressor=xz ;;
    *.nar.zst) narCompressor=zstd ;;
    *) echo "unhandled NAR compression, this test needs a matching compressor: $nar" >&2; exit 1 ;;
esac
mv "$nar" "$nar".good
mkdir -p "$TEST_ROOT/empty"
nix-store --dump "$TEST_ROOT/empty" | "$narCompressor" > "$nar"

...
# Whether this build's evaluator has the Boehm garbage collector compiled in.
#
# The sanitizer lane does not have it: `ci/gha/tests/default.nix:57` sets
# `enableGC = !withSanitizers`, because Boehm is incompatible with ASan. An
# evaluation that allocates heavily gets none of it back there, so a test that
# deliberately burns evaluator memory is unaffordable on that lane and nowhere
# else.
#
# Probed rather than keyed on the lane's name, so it tests the property that
# actually matters and stops being true on its own: `printStatistics` emits the
# `gc` object only under `#if NIX_USE_BOEHMGC` (`src/libexpr/eval.cc:3130`). The
# day the sanitizer lane stops disabling the collector, this returns true and
# every guarded test runs again with no further edit.
evaluatorHasGC () {
    local stats="${TEST_ROOT:-${TMPDIR:-/tmp}}/evaluator-gc-probe.json"
    NIX_SHOW_STATS=1 NIX_SHOW_STATS_PATH="$stats" nix eval --expr 1 > /dev/null
    jq -e 'has("gc")' < "$stats" > /dev/null
}

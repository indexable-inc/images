...
# Repeated installables should produce one result each in request order.
nix build -f multiple-outputs.nix --json a b a --no-link | jq --exit-status '
  length == 3
  and (.[0].drvPath | match(".*multiple-outputs-a.drv"))
...
if isDaemonNewer "2.29pre"; then
    # The leaf failure is preserved through the resolution layers. Depending
    # on which layer observes the dependency failure first, the client either
    # sees the propagated leaf error directly, or the depender's wrapper; when
    # the wrapper appears it must name the failed dependency count.
    if <<<"$out" grepQuiet -E "error: Cannot build '.*-x4\\.drv'"; then
...
(( "$(<<<"$out" grep -cE '^error:')" >= 2 ))
if isDaemonNewer "2.29pre"; then
    # Under keep-going every completed leaf failure is reported exactly once;
    # the depender's wrapper appears only when its layer observes the failures,
    # and must then name the failed dependency count.
    <<<"$out" grepQuiet -E "hash mismatch in fixed-output derivation '.*-x2\\.drv'"
    <<<"$out" grepQuiet -E "hash mismatch in fixed-output derivation '.*-x3\\.drv'"
    if <<<"$out" grepQuiet -E "error: Cannot build '.*-x4\\.drv'"; then

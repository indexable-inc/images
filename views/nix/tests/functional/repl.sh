...
# `ulimit -Hs` reports kilobytes; src/nix/main.cc's setStackSize() call is
# what the repl actually needs, and requests 60 MiB, not 64: the comment
# there explains that GitHub Actions' macOS runners have a hard stack
# limit slightly under 64 MiB, so the request was lowered to 60 MiB to
# fit, but this check was never updated to match. The two bugs compounded:
# comparing a kilobyte ulimit against a byte-sized constant made the
# effective threshold 64 GiB of stack, so this skipped on every machine
# that does not report "unlimited" (most non-container Linux and all of
# macOS), and even a units-only fix would still skip on a host whose hard
# limit sits in the 60-64 MiB gap the GitHub Actions comment describes.
stack_ulimit_kb="$(ulimit -Hs)"
stack_required_kb="$((60 * 1024))"
if [[ "$stack_ulimit_kb" != "unlimited" ]]; then
    ((stack_ulimit_kb < stack_required_kb)) && skipTest "repl tests cannot run on systems with stack size <${stack_required_kb}KB (${stack_ulimit_kb}KB)"
fi

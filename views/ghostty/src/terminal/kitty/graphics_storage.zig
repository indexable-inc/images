...
pub const ImageStorage = struct {
    ...
    /// Monotonic generation for image replacements. This lets consumers cache
    /// decoded image data without going stale when an image ID is reused.
    next_image_generation: u64 = 1,
    ...
    pub fn addImage(self: *ImageStorage, alloc: Allocator, img: Image) Allocator.Error!void {
        ...
        var stored = img;
        stored.generation = self.next_image_generation;
        self.next_image_generation +%= 1;
    ...
    pub fn imageByNumber(self: *const ImageStorage, image_number: u32) ?Image {
        ...
        while (it.next()) |kv| {
            if (kv.value_ptr.number == image_number) {
                if (newest == null or kv.value_ptr.generation > newest.?.generation) {
                    newest = kv.value_ptr.*;
                }
            }
        }
            ...
            struct {
                fn lessThan(
                    ...
                    if (lhs.used == rhs.used) {
                        return lhs.generation < rhs.generation or
                            (lhs.generation == rhs.generation and lhs.id < rhs.id);
                    }

...
pub fn initWasm(
    ...
    return initWasmExecutable(b, exe, "ghostty-vt.wasm");
}
...
pub fn initWebWasm(
    ...
    const exe = b.addExecutable(.{
        .name = "ghostty-vt-web",
        .root_module = zig.vt_web_c,
        .version = zig.version,
    });

    return initWasmExecutable(b, exe, "ghostty-vt-web.wasm");
}

fn initWasmExecutable(
    b: *std.Build,
    ...
    const output = patch_run.addOutputFileArg(output_name);

...
pub fn build(b: *std.Build) !void {
    ...
    if (config.target.result.cpu.arch.isWasm()) {
        const libghostty_vt_web_wasm = try buildpkg.GhosttyLibVt.initWebWasm(
            b,
            &mod,
        );
        const step = b.step(
            "web-vt-wasm",
            "Build the Ghostty web VT WASM module",
        );
        step.dependOn(libghostty_vt_web_wasm.artifact);
    }

...
/// Drop the `_` digit separators the tokenizer admits between digits.
///
/// They are purely visual and carry no value, so they come off before the
/// number is parsed -- `str::parse` rejects them. This lives beside the
/// accessors rather than in the tokenizer because the token text stays
/// lossless: the CST reproduces the source byte for byte, and only the
/// numeric value drops them.
///
/// Borrows when there is nothing to strip, which is every literal that does
/// not use a separator.
fn without_separators(text: &str) -> Cow<'_, str> {
    if text.contains('_') {
...
impl Float {
    pub fn value(&self) -> Result<f64, num::ParseFloatError> {
        without_separators(self.syntax().text()).parse()
    }
}

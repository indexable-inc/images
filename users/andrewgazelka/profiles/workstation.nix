...
in {
  ...
  # Btop
  home.file.".config/btop/btop.conf".text = textStructured.btop-btop-conf;
  ...
  # Kitty
  home.file.".config/kitty/kitty.conf".text = textStructured.kitty-kitty-conf;
  ...
  # Git
  home.file.".gitignore_global".text = textStructured.gitignore_global;
  ...
  # Tmux: personal config, managed in-store by home-manager (not a live
  # out-of-store symlink). Fleet defaults (truecolor, Claude Code 24-bit escape
  # hatch) come from the wrapped `index…tmux` package, which sources this last so
  # personal settings still win. Edits here need a `home-manager switch`.
  home.file.".tmux.conf".text = textStructured.tmux--tmux-conf;
  ...
  # IdeaVim
  home.file.".ideavimrc".text = textStructured.ideavimrc;

  # LLDB
  home.file.".lldbinit".text = textStructured.lldbinit;
  ...
  home.file.".config/tracy/tracy.ini" = {
    text = textStructured.tracy-tracy-ini;
    force = true;
  };

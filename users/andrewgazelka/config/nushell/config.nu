...
#
# One table, three shells. It used to be a record literal here and a
# hand-kept copy in ix (crates/term/config/abbreviations.json), which is
# how `c` came to mean `clear` in an ix-term pane while meaning the zoxide
# jump defined below in this shell -- along with 34 keys this file had and
# that one did not, 19 the other way, and 34 that disagreed on their
# expansion. Reading the file is what makes that class of drift impossible
# rather than merely fixed.
#
# The file lives in ix because the direction is forced: term-config embeds it
# with `include_str!` and cargo-unit builds that crate from a source filter
# holding only its own directory, so the JSON cannot live anywhere else and
# every other reader comes to it. The other two are
# ix/nix/homes/modules/shell/abbreviations.nix (zsh-abbr) and
# ix/crates/term/config (the ix-term panes).
#
# Six levels up because this file sits at
# <ix>/index/users/andrewgazelka/config/nushell/config.nu -- index is a
# submodule of ix, so the table is in the parent repo. Derived from
# `$nu.config-path` rather than written as `~/.config/nix/ix` so a worktree's
# config reads that worktree's table; home-manager deploys this file as an
# out-of-store symlink, and nushell resolves it, so the path is the real one.
#
# A relative symlink from this directory into ix would have removed the climb,
# and was rejected: index is also checked out on its own, where the link would
# dangle and take the whole config down with it.
let abbreviations = abbr flatten-abbreviations (
    open ($nu.config-path | path dirname --num-levels 6 | path join crates term config abbreviations.json)
)

-- agent.nvim (local): dispatch a line/selection to a Claude Code background agent.
--
...
-- Resumable sessions, keyed by extmark id (persists after a task finishes so the
-- DONE line stays resumable). Each entry: { id = short, full = sessionId }.
local resumable = {}
...
-- NOTE: this claude (2.1.159) only implements `claude agents`; `logs`/`attach`/
-- `stop` are advertised by `--bg` but unimplemented (they spawn a stray session
-- if invoked), so bg mode routes to the agent view to read/attach there.
...
-- Resume the agent whose task lives on the current line (or the most recent one).
--
-- `claude --resume <id>` is cwd-scoped (run it in the task's dir) and is REJECTED
-- while the session is still alive as a bg agent ("running as bg"). A bg agent
-- lingers ~1h after finishing its task, so to actually continue it we first stop
-- its (idle) process, then resume from the jobs transcript. If it's still busy
-- we don't kill it; we open the agent view so you can attach instead.
function M.resume()
  local buf = vim.api.nvim_get_current_buf()
...
function M.setup(opts)
  ...
  vim.keymap.set("n", "<leader>aa", M.dispatch, { desc = "Agent: dispatch current line" })
  vim.keymap.set("x", "<leader>aa", M.dispatch, { desc = "Agent: dispatch selection" })
  vim.keymap.set("n", "<leader>ao", M.show_output, { desc = "Agent: show last output / logs" })
  vim.keymap.set("n", "<leader>av", M.agent_view, { desc = "Agent: open `claude agents` view" })
  vim.keymap.set("n", "<leader>ar", M.resume, { desc = "Agent: resume the session on this line" })

  vim.api.nvim_create_user_command("ClaudeAgent", M.dispatch, { desc = "Dispatch current line to Claude agent" })
  vim.api.nvim_create_user_command("ClaudeAgentOutput", M.show_output, { desc = "Show last Claude agent output/logs" })
  vim.api.nvim_create_user_command("ClaudeAgentView", M.agent_view, { desc = "Open claude agents view" })
  vim.api.nvim_create_user_command("ClaudeAgentResume", M.resume, { desc = "Resume the agent on the current line" })
end

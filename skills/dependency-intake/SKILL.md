...
- A crate that reads VCS state at build time (`git_version!()`, `vergen`) fails
  in the sandbox because the source has no `.git`. Put the stable value in the
  checked source view, such as `env!("CARGO_PKG_VERSION")`. The package consumes
  that view without a source mutation hook.
...
[`packages/launchk`](packages/launchk) (Darwin-only) and its `views/launchk`
source are the reference.

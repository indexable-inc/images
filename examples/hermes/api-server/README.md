...
- [`default.ix`](default.ix) wraps the module set as a single VM in the `hermes-api` east-west group.
...
Run your frontend in a VM that joins the same `hermes-api` group (any VM with `ix.networking.groups = [ "hermes-api" ]` in its modules resolves the agent as `hermes`):

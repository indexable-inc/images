...
The other entries under `examples/` are NixOS VMs built with `index.lib.mkVm`
and applied with `ix apply`. The [`ubuntu`](ubuntu/) and [`debian`](debian/)
directories here return images instead of VM results, so example discovery
skips them; they surface as the opt-in `non-nix-*` packages above instead.

...
```sh
# From this directory (examples/python/daily-scraper in the index repo).
ix apply .#scraper
```
...
- [`default.ix`](default.ix) declares the VM with `index.lib.mkVm`.
...
`dailyScraper` module argument, so a deployment can enable S3 without forking
the service module. Store the env file with `ix secret set
daily_scraper_aws_env`, then attach it as a runtime file in

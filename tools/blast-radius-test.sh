...
# must not claim an eval runner. Assert the gate restricts label-adds to that
# label while still leaving synchronize/reopened refreshes ungated.
eval_if="$(yq '.jobs.evaluate.if // ""' "$workflow")"
if printf '%s' "$eval_if" | grep -qF "github.event.label.name == 'blast-radius'"; then

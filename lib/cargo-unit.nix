  ...
  buildWorkspace =
    ...
    let
      ...
      units = import unitsNix {
        ...
        # Per-package clippy lints (the `[lints.clippy]` table on each
        # crate, with `[workspace.lints.clippy]` inheritance) are resolved
        # inside nix-cargo-unit at render time so per-unit clippy lands
        # the exact flag set `cargo clippy` would apply to that crate.
        # `extraClippyLintArgs` stays as the nix-level policy escape hatch
        # and is appended after the per-package args.

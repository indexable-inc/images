...
in {
  ...
  ```
  */
  compiler = pkgs: args @ {jdk ? defaultJdkFor pkgs, ...}: let
  ...
  `scalac` targets.
  */
  sbt = pkgs: {jdk ? defaultJdkFor pkgs}:
  ...
  */
  mill = pkgs: {jdk ? defaultJdkFor pkgs}:
  ...
  */
  languageServer = pkgs: {jdk ? defaultJdkFor pkgs}:

{
  ...
  */
  readLoaderManifest = {
    ...
    checkedManifest =
      if missingManifestKeys != []
      then throw "ix.lib.artifacts: ${manifestPathStr} is missing required keys: ${lib.concatStringsSep ", " missingManifestKeys}"
      else if !(builtins.isList manifest.versions)
      then throw "ix.lib.artifacts: ${manifestPathStr} `versions` must be a list of game-version strings"
      else manifest;
    ...
    readLock = ver: let
      ...
      lock =
        if exists
        then lib.importJSON lockPath
        else {};
      missingLockKeys = lib.filter (key: !(lock ? ${key})) loaderLockKeys;
    in
      if !exists
      then throw "ix.lib.artifacts: ${lockPathStr} is missing; add it or remove `${ver}` from ${manifestPathStr}"
      else if missingLockKeys != []
      then throw "ix.lib.artifacts: ${lockPathStr} is missing required keys: ${lib.concatStringsSep ", " missingLockKeys}"
      else if !(lib.isString lock.url) || !(lib.hasPrefix "https://" lock.url)
      then throw "ix.lib.artifacts: ${lockPathStr} `url` must be an https:// string"
      else if !(lib.isString lock.hash) || !(lib.hasPrefix "sha256-" lock.hash)
      then throw "ix.lib.artifacts: ${lockPathStr} `hash` must be an `sha256-...` SRI string"
      else lib.filterAttrs (key: _: lib.elem key lockKeys) lock;
  in {
  ...
  fabricServers =
    lib.mapAttrs' (mcVer: lock: {
      name = "${mcVer}-fabric";
      value = mkArtifact lock;
    })
    fabricLoader.locks;
  paperServerSrcs =
    lib.mapAttrs' (mcVer: entry: {
      name = "${mcVer}-paper";
      value = entry.src;
    })
    paperServers;
...
in {
  ...
  minecraft = {
    ...
    paperPluginCatalog =
      if builtins.hasAttr defaultMinecraftVersion paperPluginCatalogs
      then paperPluginCatalogs.${defaultMinecraftVersion}
      else throw "ix.lib.artifacts.minecraft.paperPluginCatalog: no Paper plugin catalog generated for Minecraft ${defaultMinecraftVersion} (the default in packages/minecraft/catalogs/versions.nix). Run `nix run .#update-mods -- --manifest packages/minecraft/catalogs/plugins/paper/manifest.json --version ${defaultMinecraftVersion}` and commit the result.";

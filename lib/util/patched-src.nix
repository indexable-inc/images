...
# builds, the series still applies.
{
  ...
  patchFiles =
    ...
    selected;
  # The fork's derived-patch series: registry entries whose `patchDir` is this
  # call's `patchDir` (string-compared so the package's local `./patches` and
  # the check's `patchesRoot + patchDir` spelling of the same folder match).
  # A `patchNames` selection excludes them: closure gates ship upstream-shaped
  # static closures, and repo-local mechanical output has no place there.
  derivedDeclared = lib.concatMap (
    fork:
      lib.optionals (toString (patchesRoot + "/${fork.patchDir}") == toString patchDir)

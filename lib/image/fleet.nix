...
*/
{
  ...
  # so cross-VM references (`ix.endpointOf nodes.<peer> ...`) keep working
  # when VMs are wired together one `mkVm` call at a time instead of in
  # one fleet spec (ix#8306). Values are `{ config }` wrappers, i.e.
  # exactly the entries of another result's `nixosConfigurations`. Own
  # nodes win on name collisions.
  peers ? {},
}: let
  inherit
    (builtins)

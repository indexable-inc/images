...
# reproduced on demand, so the default output is still a standard OCI archive.
# The description is exposed on the result's `passthru.description`.
{
  lib,
  pkgs,
}: let
...
in {
  ...
  Returns the OCI-archive derivation. No ix verb ingests one any more
  (ENG-6044 phase 7 removed `ix image push`; `push-manifest` takes a CAS
  manifest), so today this is for other OCI tooling and as a
  `packages.<system>.<name>` output. ENG-10550 decides whether the ingest
  path comes back or this goes away with `lib/image/oci-layer.nix`.
  */
  mkNonNixImage = {
    name,
    baseImage,
  ...
  in
    ...
    # default output so consumers see a plain archive; the description is the
    # input, so the build graph is describe -> materialize. The policy is enforced

# NixOS VM boot test for the Biff 2 reading list under
# `services.biff-reading-list`.
#
# Recovered from the example's nested flake (`vmSmoke` in
# examples/biff/reading-list/flake.nix, deleted when the application moved to
# packages/biff/reading-list). The eval assertions in tests/default.nix cover
# the module contract; nothing there proves the packaged JVM application boots,
# rejects what it should, and keeps its state across a crash. This does.
#
# What it asserts: the hardening properties systemd actually applied, that the
# main process is non-root, that sqldef comes from the store rather than a
# runtime download, the rendered schema, CSRF rejection, seven invalid-input
# cases, the unique-URL upsert, HTML escaping, SQLite integrity and journal
# mode, and that the cookie secret survives both a clean restart and a SIGKILL.
#
# Its own check rather than part of the `eval` aggregate: it boots a qemu VM.
{
  ix,
  paths,
  pkgs,
...
in
  pkgs.testers.runNixOSTest {
    ...
    # The `.ix` evaluation in tests/default.nix proves the full Index platform
    # contract. A stock NixOS test VM cannot import that container-oriented
    # platform module, so declare only the option slots written by this service
    # while still using Index's real systemd hardening value.
    node.specialArgs.ix = {
      inherit (ix) systemdHardening writeRustApplication;
    };
    nodes.server = {lib, ...}: {
      imports = [(paths.modules + "/services/biff-reading-list")];
      options.ix = {
      ...
      config = {
        # `package` is set explicitly: the test node runs a stock nixpkgs
        # instance, which carries no repo overlay for `mkPackageOption` to
        # resolve the default against.
        services.biff-reading-list = {
          enable = true;
          package = packages.biff-reading-list;
        };

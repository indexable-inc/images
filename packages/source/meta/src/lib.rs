...
pub const KNOWN_SOURCE_TAGS: &[&str] = &[
    ...
    "distilled_facts",  // packages/distiller (ReasoningBank-style lessons)
    "session_outcomes", // packages/distiller (per-session verdicts)
];
...
///
/// Cloning the collection makes the returned

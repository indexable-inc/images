"""Resolve the system prompt under test, rendering the Nix prompt module if asked.

Resolution order:

1. ``--system-prompt-file <path>``: a prompt text file (used to test a candidate
   edit before it is committed).
2. ``--system-prompt-nix <path>``: render that prompt module (a dir or file
   whose import exposes ``systemPrompt``) to text with ``nix eval``.
3. ``SYSTEM_PROMPT_EVAL_PROMPT_FILE``: the prompt the Nix wrapper baked in.
4. checkout fallback: render ``packages/agent/prompt/`` next to the datasets.
...
def render_nix(nix_path: Path) -> str:
    """Render a prompt module (``packages/agent/prompt``) via ``nix eval --raw``."""
    expr = f"(import {nix_path} {{ lib = (import <nixpkgs> {{}}).lib; }}).systemPrompt"
    try:
        proc = subprocess.run(

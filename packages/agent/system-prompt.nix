...
let
  ...
  landingBanner = "Announce every landing on `origin/main` with a one-line banner: `🚀 Pushed to main: [<summary>](<commit url>)` for a direct push, or `🌸 PR merged: [<title or number>](<url>) in <duration>` for a merged PR. For a merged PR, always include `<duration>`: the wall-clock time from opening the PR to it landing on `origin/main`, which by definition includes CI and the full merge-queue wait, not just the final merge step. Compute it from the PR's own timestamps (`gh pr view <n> --json createdAt,mergedAt`, then `mergedAt - createdAt`) and render it compactly (for example `4m12s`, `1h03m`). These two emoji are a deliberate signal, the one exception to the no-decorative-emoji rule. Also play `minecraft-sound play block/amethyst/resonate1`.";

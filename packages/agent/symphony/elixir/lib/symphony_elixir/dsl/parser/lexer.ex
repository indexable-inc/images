defmodule SymphonyElixir.DSL.Parser.Lexer do
  ...
  defp do_tokenize(<<"\n", rest::binary>>, line, _col, acc), do: do_tokenize(rest, line + 1, 1, acc)

  defp do_tokenize(<<c::utf8, rest::binary>>, line, col, acc) when c in [?\s, ?\t, ?\r], do: do_tokenize(rest, line, col + 1, acc)
  ...
  defp do_tokenize(<<"<-", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 2, [tok(:larrow, "<-", line, col) | acc])

  defp do_tokenize(<<"{", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 1, [tok(:lbrace, "{", line, col) | acc])

  defp do_tokenize(<<"}", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 1, [tok(:rbrace, "}", line, col) | acc])

  defp do_tokenize(<<"[", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 1, [tok(:lbracket, "[", line, col) | acc])

  defp do_tokenize(<<"]", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 1, [tok(:rbracket, "]", line, col) | acc])

  defp do_tokenize(<<":", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 1, [tok(:colon, ":", line, col) | acc])

  defp do_tokenize(<<",", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 1, [tok(:comma, ",", line, col) | acc])

  defp do_tokenize(<<"=", rest::binary>>, line, col, acc), do: do_tokenize(rest, line, col + 1, [tok(:equals, "=", line, col) | acc])
  ...
  defp do_tokenize(<<c::utf8, _::binary>> = bin, line, col, acc) when c in ?a..?z or c in ?A..?Z or c == ?_ do
  ...
  defp scan_interp(<<c::utf8, rest::binary>>, line, col, current, acc) when c in ?a..?z or c in ?A..?Z or c in ?0..?9 or c == ?_ do
  ...
  defp scan_number(<<c::utf8, rest::binary>>, col, acc, dot?) when c in ?0..?9, do: scan_number(rest, col + 1, [<<c::utf8>> | acc], dot?)

  defp scan_number(<<".", c::utf8, rest::binary>>, col, acc, false) when c in ?0..?9, do: scan_number(rest, col + 2, [<<c::utf8>>, "." | acc], true)
  ...
  defp number_token(%{text: text, float?: true}, line, col), do: tok(:float, String.to_float(text), line, col)

  defp number_token(%{text: text, float?: false}, line, col), do: tok(:int, String.to_integer(text), line, col)

  defp scan_ident(bin, col), do: scan_ident(bin, col, [])

  defp scan_ident(<<c::utf8, rest::binary>>, col, acc) when c in ?a..?z or c in ?A..?Z or c in ?0..?9 or c == ?_, do: scan_ident(rest, col + 1, [<<c::utf8>> | acc])

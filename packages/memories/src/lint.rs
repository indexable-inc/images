...
/// A credential anywhere in the file, frontmatter included.
///
/// The one rule here with a paid-for incident behind it: a live `lin_api_*` key
/// reached at least 200 indexed chunks on this fleet. `validated.how` holds a
/// command line, exactly the shape that leaked, and unlike a transcript a memory
/// is committed on purpose.
///
/// Detection is `source_meta::sanitize::redact_secrets` applied line by line: if
/// redaction changes a line, that line holds a credential. Reusing the fleet's
/// redaction table rather than writing a second one means a pattern added there
/// is caught here too. Line by line so the diagnostic names a line; a multi-line
/// PEM block still trips on its `BEGIN` marker, which the pattern matches to end
/// of input.
fn check_secrets(memory: &Memory, diagnostics: &mut Vec<Diagnostic>) {
    let frontmatter = memory.yaml.lines().enumerate().map(|(offset, line)| {
        let line_number = offset + memory.yaml_start_line;
        LintLine { line_number, line }
    });
    let body = memory.body.lines().enumerate().map(|(offset, line)| {
        let line_number = offset + memory.body_start_line;
        LintLine { line_number, line }
    });

    for LintLine { line_number, line } in frontmatter.chain(body) {
        ...
        diagnostics.push(Diagnostic::new(
            ...
            format!(
                "line holds what the fleet's redaction table reads as a credential ({kinds}); \
                 a memory is committed on purpose, so remove it and rotate the secret",
                kinds = redaction_kinds(&redacted).join(", ")
            ),
        ));
    }
}

/// One numbered line, so the two halves of a file can be scanned as one stream.
struct LintLine<'a> {
    line_number: usize,
    line: &'a str,
}

/// The `[redacted:<kind>]` markers redaction left behind, so the diagnostic can
/// say which credential kind matched without a second copy of the table.
fn redaction_kinds(redacted: &str) -> Vec<String> {

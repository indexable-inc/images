        ...
        <article class="entry">
          ...
          {#if traced}
            ...
            <div class="entry-box entry-trace-box">
              ...
              <!-- A traced cell can also display() a DataFrame or plot: the
                   inline-trace view carries no rich outputs, so render them here. -->
              <DisplayOutputs outputs={parseExecOutputs(p.outputs)} expanded />
            </div>
          {:else if hasCode}

...
/// Short, copy-stable prefix of the store hash, shown on every build row so the
/// store path is identifiable at a glance (and so the many same-named units one
/// build produces, e.g. `ix-0.1.0`, stay distinct). Empty for an oddball path
/// with no leading hash.
export function shortHash(parts: DerivationParts): string {
  return parts.hash.replace(/-$/, '').slice(0, 7);
}

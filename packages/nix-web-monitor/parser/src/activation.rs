...
//!
//! Kept pure and `now_ms`-injected so the
...
///
/// `active` is why the panel greys out until a switch starts; `status` is a
/// human line explaining the current phase, e.g. "running",
/// "skipped (build failed)", "failed".

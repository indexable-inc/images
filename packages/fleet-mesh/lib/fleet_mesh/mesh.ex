defmodule FleetMesh.Mesh do
  @moduledoc """
  Run Elixir on other BEAM nodes over native Erlang distribution and
  `:erpc`: one specific node, a random one, the least-loaded one, or a
  fan-out across many.

  Moved verbatim from `IxMcp.Fleet`, which had one of the two hand-kept
  copies of this client (the other was test-ide's `Fleet.Mesh`); both now
  consume this module, so the env contract below has exactly one home.
  ...
  injected by whatever deploys the host application:

    * `IX_BEAM_NODES` -- comma/space/newline separated node names, e.g.
      `beamd@host-a.tailnet.ts.net beamd@host-b.tailnet.ts.net`. Empty or
      unset means no fleet is configured and the fan-out helpers return
      `{:error, :no_nodes}`.
    * `IX_BEAM_COOKIE` (or `IX_BEAM_COOKIE_FILE` pointing at a file) -- the
      shared distribution cookie. Every node in the fleet, and this node,
      must agree on it; it is a root-equivalent credential.
    * `IX_BEAM_LOCAL_NAME` -- optional. The long name this node registers as
      when distribution starts. Defaults to `mcp-ex@<hostname>` for
      historical continuity: existing deploys pin ACLs to that name.

  Distribution is started **lazily** on the first fleet call (see
  `ensure_dist/0`): a host application ships with distribution off so its
  sandboxed build and tests never try to open an epmd listen socket.
  ...
  remote node's code: a fun calling a module defined only locally fails
  there with `:undef`. Strings sidestep module resolution entirely.

  An interpreted fun also carries the md5 of the local `erl_eval` bytecode
  and the remote node raises `badfun` unless its `erl_eval` is
  byte-identical. Both ends of this fleet are nix-controlled and expected to
  share the erlang pin, so that md5 matching is the supported contract; fun
  dispatch verifies it per node up front (an MFA `:erpc` probe, itself
  version-safe, cached in `:persistent_term`) and returns
  `{:error, {:erl_eval_mismatch, ...}}` with update instructions instead of
  letting `badfun` escape mid-task.
  """
  ...
  @doc "True for the two shapes `exec/3` and friends accept as remote work."
  defguard is_code(code) when is_binary(code) or is_function(code, 0)
  ...
  A dead or unreachable node fails only this call. `:timeout` (ms) defaults
  to #{@default_timeout}.
  """

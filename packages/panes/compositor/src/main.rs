...
//! macOS side. The wire contract lives in `packages/panes/protocol`; the
...
fn run(_cli: &cli::Cli) -> anyhow::Result<()> {
    anyhow::bail!(
        "panes-compositor runs inside a Linux guest only (see packages/panes/host for the macOS side)"
    )
}

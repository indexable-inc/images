(ns com.example.todo-app.app.todos
  (:require [clojure.string :as str]
            [com.biffweb.datastar :as biff.datastar]
            [com.example.todo-app.lib.middleware :as mid]
...
(defn- normalize-title [value]
  (when-some [title (trim-to-nil value)]
...
(defn- current-todo [{{user-id :uid} :session :as ctx}]
  (first
   (biff.sqlite/execute
    ctx
    {:select [:todo/completed :todo/archived]
     :from   :todo
     :where  [:and
              [:= :todo/id (get-in ctx [:path-params :id])]
              [:= :todo/user-id user-id]]})))

(defn- toggle-todo! [{:keys [biff.fx/now] :as ctx}]
  (when-some [todo (current-todo ctx)]
    (let [completed? (if (some? (param-value ctx :completed))
...
(defn- archive-todo! [{:keys [biff.fx/now] :as ctx}]
  (when-some [todo (current-todo ctx)]
    (let [archived? (if (some? (param-value ctx :archived))
    ...
    [:div#biff-datastar-content.space-y-8
     [:section.space-y-4
      [:div.flex.flex-wrap.items-center.justify-between.gap-4
       [:div.space-y-2
        [:p {:class "text-sm font-semibold uppercase text-teal-700 tracking-[0.2em]"} "Todo App"]
        (ui/page-title "Todo App")
...
(defroute create-todo-route "/app/todos"
  ...
  (fn [{:keys [session biff.fx/now] :as req}]
    (if-some [title (normalize-title (param-value req :newtodo))]

...
/// consent flow. `gmail auth` and `gcal auth` both request this union so
/// one consent grants every Google capability the repo exposes today;
/// adding a new product means adding its scope here.

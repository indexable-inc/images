...
indexing and query pipeline lives in the Rust ``semantic-search-core`` crate;
this package is a thin PyO3 binding over it.

    hits = await semantic_search.semantic("where is retry backoff configured", ".")
    for hit in hits:
        print(hit["path"], hit["score"])

    hits = await semantic_search.grep(r"fn \\w+\\(", ".", case_sensitive=True)
    for hit in hits:
...
``is_web``. Authentication mirrors the ``semantic-search`` CLI: ``MXBAI_API_KEY``,
or the token written by ``mgrep login``.
"""

from __future__ import annotations

from ._semantic_search import __version__, grep, semantic

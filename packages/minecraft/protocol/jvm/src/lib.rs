//! JVM bindings for `mc-protocol`, declared through `unibind`.
//!
//! The same three-call boundary as the Python bindings in `../py`:
//! [`_mc_protocol_jvm::parse_address`] splits `host[:port]`,
//! [`_mc_protocol_jvm::status`] runs the full Server List Ping exchange, and
//! [`_mc_protocol_jvm::strip_format_codes`] normalizes MOTD text for
//! comparison. All protocol logic lives in the core crate; the exported
//! module only converts at the boundary, and `unibind` renders the C-ABI
...
mod _mc_protocol_jvm {
    ...
    pub struct ServerAddress {
        pub host: String,
        /// The port, an unsigned 16-bit value; Java sees the same bits as a
        /// `short`, so ports above 32767 land negative (mask with `0xffff`).
        pub port: u16,
    }
    ...
    /// Everything the boundary raises. Java sees `SlpException` (an
    /// `IOException`, the family socket-level failures throw anyway) with
    /// one subclass per failure stage.
    #[unibind::error(jvm(base = "IOException"))]
    #[derive(Debug)]
    pub enum SlpError {
        /// The address string or timeout value could not be used.
        #[unibind(jvm(name = "InvalidInputException"))]
        Input { message: String },
        /// Resolving, connecting, or socket I/O failed.
        #[unibind(jvm(name = "NetworkException"))]
        Network { message: String },
        /// The server answered with something that is not a valid status
        /// exchange.
        #[unibind(jvm(name = "ProtocolException"))]
    ...
    /// # Errors
    ///
    /// [`SlpError::Input`] when the address string cannot be parsed.
    ...
    /// # Errors
    ///
    /// [`SlpError::Input`] for a bad address or timeout,
    /// [`SlpError::Network`] for resolution/connect/socket failures, and
    /// [`SlpError::Protocol`] for a malformed status exchange.
    pub fn status(
        address: &str,

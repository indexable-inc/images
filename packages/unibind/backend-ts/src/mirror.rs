//! Generate the `#[napi(object)]` twin of a record that carries a 64-bit
//! integer.
//!
//! napi derives a record's conversions from the field types it can see, and
//! those are the user's own. A `u64` field has no napi conversion at all and
//! an `i64` field would silently cross as a lossy `number`, so a record
//! holding either crosses through a mirror struct the glue owns: same JavaScript
//! shape and same key names, but 64-bit fields declared as `BigInt`, with the
//! narrowing (and its refusal) on the way back in. Records that mention no
//! 64-bit integer keep `#[napi(object)]` on the user's struct exactly as
//! before, so the mirror only exists where it changes something.
...
/// integer directly, plus those reaching one through another record. The
/// fixpoint runs to a stop because each pass can only grow the set, which
/// is bounded by the record count.
    ...
    loop {
        ...
        for record in records {
            ...
            if record
                .fields
                .iter()
                .any(|field| convert::adapts(&field.ty, &mirrored))
            {
...
pub fn render_mirror(record: &ir::Record, ctx: &TyCtx<'_>) -> Result<TokenStream, RenderError> {
    ...
    for field in &record.fields {
        ...
        let declared = ty::decl(&field.ty, ctx, Level::Nested)?;
        let rename = field
            .names
            .ts
            .as_ref()

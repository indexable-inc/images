//! The unibind swift-backend conformance boundary.
//!
//! One `#[unibind::export]` module covering the sync phase 7 surface: echo
//! functions for every boundary type (each integer width, floats, strings,
//! paths, bytes, options, vectors, maps, and nested compositions), a record
//! round-trip, and functions whose `Result` renders as a thrown Swift enum.
//! The committed Swift runner (`swift/main.swift`) calls every case and
//! prints one `PASS <case>` line each; the nix conformance check compiles
//! and runs it against this staticlib.
//!
//! Resource close/leak and async/stream/cancellation cases are out of
//! scope here: the phase 2 IR carries them, but the swift backend renders
//! the sync surface today, so they land with its async follow-up. The
//! Python analog (packages/unibind/conformance) covers them.

/// The exported boundary.
// `backends(swift)`: this crate links swift-bridge only; the workspace
// compiles unibind with every backend feature (cargo unifies features).
#[unibind::export(backends(swift))]

...
use super::types::{self, doc_block, type_name, value_name};
...
fn field_schema(scope: &Scope<'_>, ty: &ir::Type) -> Result<String, EmitError> {
    match ty {
        ir::Type::Option(inner) => Ok(format!("{}.nullable().optional()", zod_type(scope, inner)?)),
        ty => zod_type(scope, ty),
    }
}

/// The Zod schema of a value inside a record, mirroring [`types::ts_type`] at
/// [`super::types::Level::Nested`] -- the only level a record field reaches.
fn zod_type(scope: &Scope<'_>, ty: &ir::Type) -> Result<String, EmitError> {
    Ok(match ty {
        ...
        // Bytes only ever reach a record nested, where they cross as a plain
        // number array (`Array<number>`), never as a `Buffer`.
        ir::Type::Bytes { .. } => "z.array(z.number().int())".to_owned(),
        ir::Type::Option(inner) => format!("{}.nullable()", zod_type(scope, inner)?),
        ir::Type::Vec(inner) => format!("z.array({})", zod_type(scope, inner)?),
        ir::Type::Map { key, value } => {
            ...
            format!("z.record(z.string(), {})", zod_type(scope, value)?)
        }

{
  ...
  systemPrompt ? lib.concatStringsSep "\n\n" [
    ...
    "Always open a GitHub issue in `indexable-inc/index` when using index is subpar — a confusing or misleading tool surface, output that floods your context, a wedged kernel, a correction that you did the wrong thing, or a task that worked but has a plainly better implementation. One concrete observation per issue: what you expected, what happened, and the smallest change that would have helped."
  ],

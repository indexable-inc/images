...
writeNushellApplication {
  ...
  text = ''
    ...
    # `devChannelsGateAnchor` in ./default.nix. The gate patch is currently
    # commented out there, so this bump cannot fail on a stale anchor; the
    # reminder stays because the anchors it names silently rot with every
    # release, and whoever re-enables the patch needs to know they are stale
    # rather than trusting four strings that look maintained.
    def print_anchor_reminder [] {
      ...
      print "  byte-patch target counted against the pinned binaries. The gate patch that"
      print "  consumed it is commented out today, so nothing checks the anchors and this"
      print "  bump just made them one release staler. Re-derive all four from the newly"
      print "  pinned binaries before turning the patch back on; with it on, the byte"
      print "  patcher's `expect = 1` gate catches drift as `COUNT DRIFT ... expected 1,"
      print "  found 0`."

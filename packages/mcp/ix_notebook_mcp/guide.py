...
NAMESPACE = (
    ...
    "again next turn. Bind expensive or large outputs to names (`text = Path(p).read_text()`, "
    "`df = ...`) instead of only printing or returning them, so later calls can inspect, slice, "
    "or filter that same object without recomputing. Persistence cuts both ways: a "
...
PAGING = (
    ...
    "small final expression, e.g. `text = Path(p).read_text(); text[:2000]`, so both the named "
    "object and the pageable job survive for follow-up calls; history() lists recent runs."
)
...
BLOCKING = (
    ...
    "for requests, `await nu('^cmd ...')` for an external binary; the search helpers "
    "`grep`/`find`/`spotlight` shell out too, so `await` them), and run anything slow as a "
    "background job you poll, never inline. To run an external program, reach for `nu(...)` "
    "rather than a hand-rolled `asyncio.create_subprocess_exec/_shell` + `communicate()`, which "
    "runs synchronously on the one loop and hands back ANSI-corrupted output. Run independent "
    "non-mutating commands concurrently inside one cell with `asyncio.gather` or "
    "`asyncio.TaskGroup`."
)

MACHINE_READABLE = (
    "Optimize output for the machine reading it: your tool reply is model context, so small, "
    "structured, and lossless beats pretty. Return values and let the renderer split the human "
    "and model views; never print a human-styled rendering (a box-drawn table, a padded ASCII "
    "layout, ANSI color) into stdout, where it burns tokens and clips the data."
)
...
PYTHON_FIRST = (
    ...
    "themselves, so skip the glue: `pathlib` to read, list, and stat files "
    "(`Path(p).read_text()`, `.iterdir()`, `.glob()`, `.stat()`) instead of "
    "`cat`/`ls`/`tail`/`stat`, `psutil` for process questions instead of `ps`/`lsof`, "
    "`datetime` instead of `date`, `httpx` instead of `curl`, and `githubkit` instead of the "
    "`gh` CLI (typed async GitHub client; mint it once per session with `from githubkit import "
    "GitHub; gh = GitHub((await nu('^gh auth token')).strip())` and reuse it: a direct API call "
    "returns structured data in well under a second where every `gh` fork costs several). "
    "Reach for `nu(...)` ONLY when a real external program is genuinely needed (git plumbing, "
    "`gh auth token`, a CLI with no Python-native equivalent), and even then prefer one program "
    "per call with a `--json` flag, parsed in Python, over any pipeline."
)

NO_SHELL = (
    "Do NOT hand-roll shell through Python either: never `subprocess.run`, `os.system`, or "
    ...
    "silently mangling and truncating the very tokens you parsed). To search, `await "
    "grep(pattern)` / `await find(...)`: they wrap ripgrep/fd, run OFF the loop as a separate "
    "timeout-bounded process a runaway cannot wedge, respect `.gitignore`, and return composable "
    "polars frames (`.filter`/`.sort`/`.group_by`/`.head`); on macOS `await spotlight(query)`; "
    "for meaning-based recall across a corpus `import search`. To edit a file, "
    "`view.edit(path, old, new)`."
)

NU = (
    "`nu` is the ONE shell-out path, reserved for a real external program (the Python-first "
    "rule above; the old `sh()`/`zsh()` are retired and raise a migration hint). It runs "
    "NUSHELL, not bash, and pasted bash is the classic trap, so do not write shell you do not "
    "need: keep each call to ONE external command, prefixed with `^` so a nushell builtin "
    "cannot shadow it (`await nu('^git status --short')`, `await nu('^ps aux')`; bare `ps`, "
    "`open`, `sort`, `kill` hit nushell builtins, `2>/dev/null` becomes a literal argument, "
    "and `&&` does not chain). Prefer a `--json` flag and parse the returned stdout string in "
    "Python; externals run color-free by default, so that output decodes directly. A failing "
    "command raises `NuError` carrying nushell's own diagnostic; when a non-zero exit is an "
    "answer (a grep with no match), pass `check=False` to get `NuResult(result, exit_code)` "
    "instead of a raise. The engine is persistent per session (a `cd` sticks across calls; "
    "pass `cwd=` to be explicit, `nu.reset()` to clear), and `timeout=` cannot kill an "
    "external it already spawned, so run a genuinely long command as a background job you poll."
)
...
CELLS = (
    ...
    "there is no `dashboard()` function to call). When a result is worth showing the human, add it there with "
    "`cells.add(value, title=...)` (a DataFrame, a figure, an htpy element). Treat cells as the FINAL "

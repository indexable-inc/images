...
NU = (
    ...
    "json')`). Pass each standalone value as its own token (`^cmd $env.VALUE`). "
    "A compound argument needs Nushell interpolation: `$\"title=($env.ISSUE_TITLE)\"`; bare "
    "`title=$env.ISSUE_TITLE` stays literal. Externals "
...
DELEGATE = (
    ...
    "and the result in CAS); keep the main thread free with `job = jobs.spawn(s.result(), "

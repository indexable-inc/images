...
NU = (
    ...
    "json')`); pass arguments as separate tokens so nushell never re-parses prose. Externals run "
    "with color OFF by default (`NO_COLOR=1`, forcing vars zeroed), so `gh ... --json | from "
    "json` decodes cleanly with no `with-env` dance; pass `env=` to re-enable color for a call. "
    "The engine is embedded and PERSISTENT, a REPL: a `let` or a `def` "
    ...
    "own diagnostic (span + 'did you mean'), so read it and fix the pipeline. It evaluates off "

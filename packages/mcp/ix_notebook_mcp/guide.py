...
NU = (
    ...
    "json')`); pass arguments as separate tokens so nushell never re-parses prose. An argument "
    "LIST runs one external command with NO shell parsing: `await nu(['gh', 'pr', 'create', "
    "'--title', title, '--body', body])` renders each element as a string literal, so spaces, "
    "quotes, and multiline bodies never need hand-quoting. Externals "
...
DELEGATE = (
    ...
    "the shared journal; the weave app fulfills them as a live interactive session - "
    "visible and interruptible in the Constellation - and publishes the terminal outcome to "
    "the journal. `harness=` picks the CLI per dispatch: 'claude' (the default) or 'codex' "
    "(the OpenAI Codex CLI); 'omp' is reserved and raises for now. With `harness='codex'`, "
    "`model=` takes any Codex model id - current catalog: gpt-5.6-sol (recommended for "
    "delegated task work), gpt-5.6-terra, gpt-5.6-luna, plus the older gpt-5.5 / gpt-5.4 / "
    "gpt-5.2 - or omit it to use the codex CLI's configured default; `effort=` sets the "
    "reasoning effort (low/medium/high/xhigh on all models, max/ultra on sol and terra only) "
    "and is codex-only, rejected with `harness='claude'`. Reusing an agent `name=` across "
    "dispatches is latest-wins on the agent entity, so vary config on fresh names (the default "
    "auto-name is always fresh). `await weave.result(task)` blocks until the task finishes and "

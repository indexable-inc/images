...
# Import package == distribution name here: `import polars_sftp`.
PKG = "polars_sftp"
DIST = "polars_sftp"
DIST_NAME = "polars-sftp"
SO_NAME = "_polars_sftp.abi3.so"
SOURCE_FILES = ["__init__.py", "_polars_sftp.pyi", "py.typed"]
...
def build_wheel(
    *,
    ...
    files[f"{dist_info}/METADATA"] = (
        ...
        f"Name: {DIST_NAME}\n"
        f"Version: {version}\n"
        "Summary: Polars IO source for remote files over SFTP. "
        "Imported as `polars_sftp`; use scan_sftp().\n"
        "Author: indexable\n"
        "Requires-Python: >=3.11\n"
        # The Rust polars (0.53) the cdylib links is ABI-matched to one Python
        # polars minor; pyo3-polars offers no cross-version stability. Pin tight.
        "Requires-Dist: polars>=1.40,<1.41\n"
    ).encode()

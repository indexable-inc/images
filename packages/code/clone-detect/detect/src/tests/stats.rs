...
use crate::types::generated_header;
...
#[test]
fn authored_groups_rank_ahead_of_generated_groups() {
    let authored = CloneGroup {
        clone_type: Kind::Type1,
        fragments: vec![fragment("authored.rs", 1, 3), fragment("other.rs", 1, 3)],
    };
    let mut generated_fragments = vec![
        fragment("generated.rs", 1, 20),
        fragment("other-generated.rs", 1, 20),
    ];
    for fragment in &mut generated_fragments {
        fragment.generated = true;
    }
    let generated = CloneGroup {
        clone_type: Kind::Type1,
        fragments: generated_fragments,
    };
    let mut groups = vec![generated, authored];

    rank_by_impact(&mut groups);

    assert!(!groups[0].is_generated());
    assert!(groups[1].is_generated());
}

#[test]
fn generated_header_is_bounded_by_lines_and_bytes() {
    assert!(generated_header("// GENERATED BY schema\nfn main() {}"));
    assert!(!generated_header(&format!("{}// generated by tail", "x".repeat(4096))));
    assert!(!generated_header(&format!("{}// generated by line nine", "x\n".repeat(8))));
}

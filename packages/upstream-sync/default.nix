...
# intent in lib/fork-packages.nix, generated live state in each series'
# upstream-status.json, double gating of the outward PR-opening act, and the
# `drift` companion report the fork-sync cron consumes).
#
...
# with --mapping) and the runtime tools, including the pinned `nix` client for
# the closure-gate preflight (same posture as lib/fork-updater.nix's
# updateScript) and the sibling `upstream-pr` mechanism it delegates to.
{
  ix,
  lib,
  formats,
  ...
  symlinkJoin {
    ...
    postBuild = ''
      ...
      wrapProgram $out/bin/upstream-sync \
        --prefix PATH : ${lib.makeBinPath [git gh coreutils nix upstream-pr]} \

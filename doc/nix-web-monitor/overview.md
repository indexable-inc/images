...
- **`server`** (`nix-web-monitor`): the `axum` binary that spawns `nix`, feeds
  ...
  browsers over a WebSocket. Wrapped with a built Svelte site.

The state-machine, transport, and dependency-resolution mechanics
...
otherwise-unsized "copying to the store" source. The browser feed is plain
`ws://` on the page's
...
- `serve`: run the monitor as a standalone server with no wrapped Nix command,
  until interrupted. The machine-builds view (fed by the patched-nix
  `nix store builds --json` poller) is the content; the build tree shows a
  "no wrapped command"
...
- `MonitorState` (`:276`): the accumulating state machine. `new(command)`,
  ...
  `record_closure`, `set_global`, `set_activity_size`.

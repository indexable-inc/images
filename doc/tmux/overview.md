...
(`packages/terminal/tmux/default.nix:12-23`):
...
- `symlinkJoin` (not a bare wrapper) folds tmux's separate `man` output into the
  ...
  (`packages/terminal/tmux/default.nix:14-19`).
- `meta.outputsToInstall` is forced to `[ "out" ]`: this derivation has only
  ...
  (`packages/terminal/tmux/default.nix:27-30`).
- `meta.description` is suffixed "with modern truecolor defaults baked in" and
  `mainProgram = "tmux"` (`packages/terminal/tmux/default.nix:24-26`).
...
(`packages/terminal/tmux/tmux.conf:33-35`). Key settings:

- Truecolor: `default-terminal "tmux-256color"`, `terminal-features ",*:RGB"`
  (24-bit passthrough) and `,*:usstyle` (curly/colored underlines)
  (`packages/terminal/tmux/tmux.conf:9-11`). Without an RGB terminfo tmux quantizes
...
- `CLAUDE_CODE_TMUX_TRUECOLOR 1` set in the tmux environment
  (`packages/terminal/tmux/tmux.conf:16`): Claude Code clamps its own rendering to
...
- Responsiveness: `escape-time 10`, `focus-events on`, `repeat-time 600`
  (`packages/terminal/tmux/tmux.conf:19-21`).
- Comfort: `mouse on`, `history-limit 50000`, `set-clipboard on` (OSC 52),
  ...
  `allow-passthrough on` (`packages/terminal/tmux/tmux.conf:24-31`).

## Build and wiring

- Flake output: `nix run .#tmux` / `nix build .#tmux`. `package.nix` sets
  `packageSet = true`, `flake = true` (`packages/terminal/tmux/package.nix:1-5`); no

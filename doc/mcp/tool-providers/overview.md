...
| [`hotels`](../../../packages/mcp/src/hotels/hotels/__init__.py) | search + cross-compare hotels across Expedia/Google/Booking/Kayak into polars by driving the running browser over CDP | `await hotels.search(where, check_in=, check_out=, amenities=, sort=)`, `compare(...)` (fan out + align same hotel), `amenities(...)` | pluggable backends (`backends/`) parse each site's cards into one pydantic `Hotel`; raw CDP (not `browser`) to reach recent Chrome; no account, no gate |
...
| [`nox_autotriage`](../../../packages/mcp/src/nox_autotriage/nox_autotriage/__init__.py) | convert a nox conformance JSON report into deduped Linear issues via `linear.triage` | `findings_from_conformance`, `config_from_env`, `run`; `python -m nox_autotriage --report PATH` | a CI CLI, not a kernel helper (not in the registry); `DRY_RUN=1` default |
